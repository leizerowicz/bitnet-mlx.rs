...
mod metal_integration_tests {
    ...
    use bitnet_core::tensor::acceleration::{MetalAccelerator, AccelerationBackendImpl};
    ...
    fn test_metal_accelerator_creation() {
        ...
        match result {
            ...
            Err(e) => {
                if is_metal_available() {
                    panic!("Failed to create Metal accelerator on available system: {}", e);
                } else {
    ...
    fn test_metal_accelerator_initialization() {
        ...
        let mut accelerator = create_metal_accelerator()
            .expect("Failed to create Metal accelerator");
        ...
        match result {
            Ok(()) => {
                ...
                // Cleanup
                accelerator.cleanup().expect("Failed to cleanup accelerator");
            }
            Err(e) => {
    ...
    fn test_metal_matrix_multiplication() {
        ...
        match result {
            Ok((result_tensor, metrics)) => {
                println!("✅ Metal matmul completed in {:.3}ms", elapsed.as_secs_f64() * 1000.0);
                ...
                let result_data = result_tensor.as_slice::<f32>()
                ...
                for (i, (&actual, &expected)) in result_data.iter().zip(expected.iter()).enumerate() {
                    ...
                    assert!(diff < 1e-5, "Element {} differs: {} vs {} (diff: {})", i, actual, expected, diff);
                }
    ...
    fn test_metal_element_wise_operations() {
        ...
        match result {
            Ok((result_tensor, metrics)) => {
                println!("✅ Metal add completed in {:.3}ms", add_elapsed.as_secs_f64() * 1000.0);
                ...
                for (i, (&actual, &expected)) in result_data.iter().zip(expected.iter()).enumerate() {
                    let diff = (actual - expected).abs();
                    assert!(diff < 1e-5, "Add element {} differs: {} vs {} (diff: {})", i, actual, expected, diff);
                }
        ...
        match result {
            Ok((result_tensor, metrics)) => {
                println!("✅ Metal mul completed in {:.3}ms", mul_elapsed.as_secs_f64() * 1000.0);
                ...
                for (i, (&actual, &expected)) in result_data.iter().zip(expected.iter()).enumerate() {
                    let diff = (actual - expected).abs();
                    assert!(diff < 1e-5, "Mul element {} differs: {} vs {} (diff: {})", i, actual, expected, diff);
                }
    ...
    fn test_metal_performance_benchmark() {
        ...
        let memory_stats = accelerator.get_memory_stats()
    ...
    fn test_metal_memory_management() {
        ...
        let initial_stats = accelerator.get_memory_stats()
        ...
        let final_stats = accelerator.get_memory_stats()
            .expect("Failed to get final memory stats");
        println!("Final memory stats: {:?}", final_stats);

        // Check that memory is properly managed
        assert!(final_stats.allocation_efficiency >= 0.0, "Allocation efficiency should be non-negative");
        println!("✅ Memory management test completed");

        accelerator.cleanup().expect("Failed to cleanup accelerator");
        
        // Get stats after cleanup
        let cleanup_stats = accelerator.get_memory_stats()
