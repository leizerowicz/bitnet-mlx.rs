...
mod bitnet_metal_tests {
            ...
            match compiler_result {
                Ok(compiler) => {
                    ...
                    for shader_file in &shader_files {
                        let shader_path = std::path::PathBuf::from("bitnet-core/src/metal/shaders").join(shader_file);
                            ...
                            match compile_result {
                                Ok(compiled_shader) => {
                                    println!("✓ Compiled {}: {} functions", 
                                           shader_file, compiled_shader.function_names.len());
                                    
                                    // Verify expected functions exist
                                    verify_shader_functions(shader_file, &compiled_shader.function_names);
                                }
    ...
    fn test_shader_function_enum() {
        ...
        assert_eq!(BitNetShaderFunction::BitLinearForward.pipeline_key(), "bitlinear::bitlinear_forward");
        
        // Test quantization functions
        assert_eq!(BitNetShaderFunction::QuantizeWeights1Bit.shader_name(), "quantization");
        assert_eq!(BitNetShaderFunction::QuantizeWeights1Bit.function_name(), "quantize_weights_1bit");
        assert_eq!(BitNetShaderFunction::QuantizeWeights1Bit.pipeline_key(), "quantization::quantize_weights_1bit");
        
        // Test activation functions
        assert_eq!(BitNetShaderFunction::ReluForward.shader_name(), "activation");
        assert_eq!(BitNetShaderFunction::ReluForward.function_name(), "relu_forward");
        assert_eq!(BitNetShaderFunction::ReluForward.pipeline_key(), "activation::relu_forward");
    ...
    fn test_compute_encoder_creation(shaders: &BitNetShaders, device: &metal::Device) {
        ...
        for function in &encoder_functions {
            let encoder_result = shaders.create_compute_encoder_with_pipeline(&command_buffer, *function);
    ...
    fn test_bitlinear_compute_operations() {
        ...
        if let Ok(device) = device_result {
            ...
            match shaders_result {
                ...
                Err(e) => {
                    println!("Skipping BitLinear operations test (shader loading failed): {}", e);
                }
            }
        } else {
    ...
    fn test_bitlinear_forward(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        // Create test data
        let batch_size = 2u32;
        let input_size = 4u32;
        ...
        match (input_buffer_result, weights_buffer_result, bias_buffer_result, output_buffer_result) {
    ...
    fn test_weight_binarization(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        ...
        match (weights_fp_buffer_result, weights_binary_buffer_result, scale_factors_buffer_result) {
    ...
    fn test_activation_quantization(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        ...
        match (activations_buffer_result, quantized_buffer_result, scale_buffer_result, zero_buffer_result) {
    ...
    fn test_quantization_compute_operations() {
        ...
        if let Ok(device) = device_result {
            ...
            match shaders_result {
                ...
                Err(e) => {
                    println!("Skipping quantization operations test (shader loading failed): {}", e);
                }
            }
        } else {
    ...
    fn test_1bit_weight_quantization(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
    ...
    fn test_8bit_activation_quantization(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        let activations = vec![0.1f32, 0.5, -0.3, 0.8, -0.1, 0.2, 0.9, -0.7];
    ...
    fn test_dynamic_quantization(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
    ...
    fn test_mixed_precision_matmul(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        let m = 2u32;
        let n = 3u32;
        let k = 4u32;
        ...
        if let Ok(device) = device_result {
            ...
            match shaders_result {
                Ok(shaders) => {
                    ...
                    test_activation_function(&device, &command_queue, &shaders, BitNetShaderFunction::ReluForward, "ReLU");
                    test_activation_function(&device, &command_queue, &shaders, BitNetShaderFunction::GeluForward, "GELU");
                    test_activation_function(&device, &command_queue, &shaders, BitNetShaderFunction::SwishForward, "Swish");
                    test_activation_function(&device, &command_queue, &shaders, BitNetShaderFunction::SigmoidForward, "Sigmoid");
                    test_activation_function(&device, &command_queue, &shaders, BitNetShaderFunction::TanhForward, "Tanh");
                ...
                Err(e) => {
                    println!("Skipping activation operations test (shader loading failed): {}", e);
                }
            }
        } else {
    ...
    fn test_fused_activation(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
    ...
    fn test_softmax_operation(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        let batch_size = 2u32;
    ...
    fn test_layer_norm_operation(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        let batch_size = 2u32;
        let feature_size = 4u32;
        let input_data = vec![1.0f32, 2.0, 3.0, 4.0, 0.5, 1.5, 2.5, 3.5]; // 2x4
    ...
    fn test_compute_performance_benchmarks() {
        ...
        if let Ok(device) = device_result {
            ...
            match shaders_result {
                ...
                Err(e) => {
                    println!("Skipping performance benchmarks (shader loading failed): {}", e);
                }
            }
        } else {
    ...
    fn benchmark_bitlinear_operations(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        ...
        for &(input_size, output_size) in &sizes {
            ...
            println!("✓ BitLinear {}x{} (batch={}): {:.2}ms avg, {:.1} ops/sec",
                   input_size, output_size, batch_size, avg_time.as_millis(), ops_per_sec);
        }
    }

    fn benchmark_quantization_operations(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        ...
        for &size in &sizes {
            ...
            println!("✓ Quantization {} elements: {:.2}ms avg, {:.1}M elem/sec",
                   size, avg_time.as_millis(), throughput);
        }
    }

    fn benchmark_activation_operations(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        let sizes = [1024, 4096, 16384, 65536];
        let activations = [
        ...
        for &(function, name) in &activations {
            for &size in &sizes {
                ...
                println!("✓ {} {} elements: {:.2}ms avg, {:.1}M elem/sec",
                       name, size, avg_time.as_millis(), throughput);
            }
        }
    }

    fn benchmark_scaling_performance(device: &metal::Device, command_queue: &metal::CommandQueue, shaders: &BitNetShaders) {
        ...
        for &threadgroup_size in &threadgroup_sizes {
            ...
            println!("  Threadgroup size {}: {:.2}ms avg", threadgroup_size, avg_time.as_millis());
        }
    }
}
...
mod non_metal_tests {
    ...
    fn test_unsupported_platform_bitnet_compute() {
        println!("BitNet compute pipeline tests skipped - not on macOS or Metal feature not enabled");
