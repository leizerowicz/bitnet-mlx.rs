    ...
    for text in test_texts {
        ...
        let has_unknown_words = text.split_whitespace().any(|word| {
            ![
                ...
                "<pad>", "<unk>", "<cls>", "<sep>", "<mask>", "<bos>", "<eos>", "café", "naïve",
                "🌍", "测试",
            ]
            ...
            || has_unknown_words
        {
            ...
            assert!(
                ...
                "Expected <unk> in decoded text for: '{text}'"
            );
...
fn test_tokenizer_batch_encoding_correctness() {
    ...
    for (i, text) in test_texts.iter().enumerate() {
        ...
        assert_eq!(
            batch_tokens[i], individual_tokens,
    ...
    for (i, tokens) in batch_tokens.iter().enumerate() {
        ...
        assert_eq!(
            *tokens, re_encoded,
            "Batch decode-encode inconsistency for original text: '{}'",
            test_texts[i]
        );
    }
}
...
fn test_tokenizer_consistency_across_operations() {
    ...
    for (i, batch_result) in batch_results.iter().enumerate() {
        assert_eq!(
            batch_result, first_result,
            "Batch result {i} differs from individual"
        );
    }
}
...
mod mlx_quantization_tests {
    use super::*;
    use bitnet_core::mlx::{mlx_dequantize, mlx_quantize};
    use mlx_rs::Array;
...
fn test_end_to_end_text_processing_pipeline() {
    ...
    for (i, decoded) in decoded_texts.iter().enumerate() {
        ...
        let has_known_content = known_words.iter().any(|&word| {
            original_words
                .iter()
                .any(|&orig_word| orig_word.to_lowercase().contains(word))
    ...
    for input in edge_cases {
        ...
        if input.trim().is_empty() {
            ...
            assert_eq!(
                tokens,
                Vec::<u32>::new(),
                "Whitespace-only input should produce empty tokens: '{input}'"
            );
        } else {
            // Non-empty strings should encode successfully
            assert!(
                encode_result.is_ok(),
                "Should successfully encode: '{input}'"
            );

            let tokens = encode_result.unwrap();
            assert!(
                !tokens.is_empty(),
                "Non-empty input should produce non-empty tokens: '{input}'"
            );
            ...
            assert!(
                decode_result.is_ok(),
                "Should successfully decode tokens for: '{input}'"
            );
        }
    }
}
