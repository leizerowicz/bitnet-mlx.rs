...
fn test_manual_cleanup() {
    ...
    let result = manager.cleanup_selective(
...
fn test_cleanup_strategy_registration() {
    ...
    let result = manager.register_cleanup_strategy(
        CleanupStrategyType::Idle,
        Box::new(custom_strategy),
    );
    assert!(result.is_ok());
}
...
fn test_cleanup_performance() {
    ...
    assert!(total_duration < Duration::from_secs(5), "Cleanup operations should complete quickly");
