...
mod integration_tests {
    ...
    fn test_compute_pipeline_memory_integration() {
        ...
        if let Ok(device) = device_result {
            ...
            println!("Skipping compute pipeline memory integration test - no Metal device available");
        }
    }
    ...
    fn test_pooled_buffer_compute_operations(
        ...
        println!("  ✓ Pool stats after compute operations: cache_hits={}, cache_misses={}", 
               stats.cache_hits, stats.cache_misses);
    }

    fn test_memory_tracking_compute_operations(device: &metal::Device, manager: &CommandBufferManager) {
                                ...
                                if begin_result.is_ok() {
                                    ...
                                    match encoder_result {
                                        Ok(encoder) => {
                                            ...
                                            encoder.end_encoding();
                                            println!("  ✓ Compute encoder with tracked resource created");
                                        }
                                        Err(e) => println!("  Failed to create compute encoder: {}", e),
                                    }
                                }
    ...
    fn test_compute_memory_cleanup(
        ...
        println!("  ✓ Final stats - Command buffers: {}, Pool buffers: {}", 
    ...
    fn test_compute_pipeline_tensor_integration() {
        ...
        if let Ok(metal_device) = device_result {
            ...
            println!("Skipping compute pipeline tensor integration test - no Metal device available");
        }
    }
    ...
    fn test_tensor_compute_operations(bitnet_device: &Device, metal_device: &metal::Device) {
        ...
        match tensor_result {
            Ok(tensor) => {
                println!("  ✓ Created tensor for compute operations: {:?}", tensor.shape());
    ...
    fn test_bitnet_tensor_compute_integration(bitnet_device: &Device, metal_device: &metal::Device) {
        ...
        match tensor_result {
            Ok(bitnet_tensor) => {
                ...
                println!("  ✓ BitNet tensor metadata: shape={:?}, dtype={:?}", 
    ...
    fn test_bitnet_tensor_compute_operations(tensor: &BitNetTensor, metal_device: &metal::Device) {
        ...
        match data_result {
            Ok(data) => {
                println!("    ✓ Extracted BitNet tensor data: {} elements", data.len());
    ...
    fn test_tensor_lifecycle_compute_integration(bitnet_device: &Device, metal_device: &metal::Device) {
    ...
    fn test_end_to_end_bitnet_workflows() {
        ...
        if let Ok(metal_device) = device_result {
            ...
            match shaders_result {
                ...
                Err(e) => {
                    println!("Skipping end-to-end workflows (shader loading failed): {}", e);
                }
            }
        } else {
        ...
        if let Ok(encoder) = encoder_result {
            ...
            match (original_result, dequantized_result) {
                (Ok(original), Ok(dequantized)) => {
                    ...
                    let signs_match = original.iter().zip(dequantized.iter())
                ...
                match cb_id_result {
                    Ok(cb_id) => {
                        ...
                        match add_result {
                            ...
                            Err(e) => println!("    Failed to add pooled buffer as resource: {}", e),
                        }
                    }
                    Err(e) => println!("    Failed to create command buffer: {}", e),
                }
            ...
            Ok(compiler) => {
                ...
                match shaders_result {
                    Ok(shaders) => {
                        ...
                        let pipeline_result = shaders.get_pipeline(BitNetShaderFunction::ReluForward);
    ...
    fn test_cross_system_error_handling(device: &metal::Device) {
        ...
        let large_buffer_result = pool.get_buffer(usize::MAX, metal::MTLResourceOptions::StorageModeShared);
                ...
                match cb_id_result {
                    Ok(cb_id) => {
                        ...
                        if begin_result.is_ok() {
                            ...
                            match encoder_result {
                                Ok(_encoder) => {
                                    println!("    ✓ Compute system handled memory error gracefully");
                                }
        ...
        match shaders_result {
            Ok(shaders) => {
                ...
                let invalid_pipeline_result = shaders.get_pipeline(BitNetShaderFunction::BitLinearForward);
    ...
    fn test_memory_compute_performance(device: &metal::Device) {
        ...
        for i in 0..num_operations {
            ...
            let buffer_result = pool.get_buffer(buffer_size, metal::MTLResourceOptions::StorageModeShared);
            if let Ok(buffer) = buffer_result {
        ...
        println!("    ✓ Memory-compute integration: {:.1} ops/sec", ops_per_sec);
        
        // Check pool statistics
        let stats = pool.get_stats();
        println!("    ✓ Pool efficiency: {:.1}% cache hit rate",
    ...
    fn test_tensor_compute_performance(device: &metal::Device) {
        ...
        println!("    ✓ Tensor-compute integration: {:.1} ops/sec", ops_per_sec);
    }
    ...
    fn test_shader_compute_performance(device: &metal::Device) {
        ...
        match shaders_result {
            Ok(shaders) => {
                ...
                println!("    ✓ Shader-compute integration: {:.1} ops/sec", ops_per_sec);
            }
