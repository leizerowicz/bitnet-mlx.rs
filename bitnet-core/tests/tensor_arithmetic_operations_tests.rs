...
mod arithmetic_tests {
    ...
    fn test_broadcasting_incompatible_error() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result = add(&a, &b);
        assert!(result.is_err(), "Expected error for incompatible broadcasting");
    ...
    fn test_division_by_zero_error() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(result2.is_err(), "Expected error for in-place division by zero");
    ...
    fn test_inplace_shape_mismatch_error() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(result.is_err(), "Expected error for in-place shape mismatch");
    ...
    fn test_power_unsupported_dtype_error() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(result.is_err(), "Expected error for unsupported power operation on I32");
