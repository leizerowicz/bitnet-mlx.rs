...
fn test_tensor_memory_tracking() {
    ...
    println!("Active allocations: {}, Current allocated: {}",
...
fn test_transpose_2d_comprehensive() {
    ...
    for device in &devices {
        ...
        let square_candle = square_tensor.to_candle()
...
fn test_transpose_higher_dimensional() {
    ...
    let candle_4d = tensor_4d.to_candle()
        .expect("Failed to convert 4D tensor");
    ...
    let candle_5d = tensor_5d.to_candle()
        .expect("Failed to convert 5D tensor");
...
fn test_broadcasting_operations() {
    ...
    let candle_2x3 = tensor_2x3.to_candle()
        .expect("Failed to convert 2x3 tensor");
    
    let candle_1x3 = tensor_1x3.to_candle()
...
fn test_quantization_round_trip_accuracy() {
    ...
    let candle_f32 = f32_tensor.to_candle()
        .expect("Failed to convert to candle");
...
fn test_device_migration_error_conditions() {
    ...
    let same_device_result = tensor.to_device(source_device, &pool)
...
fn test_device_enumeration_and_discovery() {
    ...
    assert!(matches!(devices[0], Device::Cpu), "First device should be CPU");
    ...
    assert_eq!(devices.len(), expected_device_count,
    ...
    for device in &devices {
        ...
        assert!(!description.is_empty(), "Device description should not be empty");
...
fn test_device_capability_detection() {
    ...
    if metal_available {
        ...
        assert!(metal_device.is_ok(), "Metal device creation should succeed when available");
        ...
        let metal_device = bitnet_core::device::get_metal_device();
        assert!(metal_device.is_err(), "Metal device creation should fail when not available");
        
        // Test Metal device name returns None when not available
        let device_name = bitnet_core::device::get_metal_device_name();
        assert!(device_name.is_none(), "Metal device name should be None when not available");
    }
    
    // Test auto device selection
    let auto_device = bitnet_core::device::auto_select_device();
    if metal_available {
        assert!(matches!(auto_device, Device::Metal(_)),
...
fn test_device_specific_memory_management() {
    ...
    for device in &devices {
        println!("Testing memory management on device: {}",
        ...
        match BitNetTensor::zeros(&[large_size], BitNetDType::F32, device, &pool) {
            Ok(large_tensor) => {
                println!("  Large allocation succeeded: {} bytes", large_tensor.size_bytes());
...
fn test_device_error_handling_scenarios() {
    ...
    if !bitnet_core::device::is_metal_available() {
        ...
        assert!(metal_result.is_err(), "Metal device creation should fail when not available");
    ...
    assert!(same_device_result.is_ok(), "Same device migration should always succeed");
    ...
    let large_result = BitNetTensor::zeros(&[very_large_size], BitNetDType::F32, &cpu_device, &pool);
    match large_result {
        Ok(_) => {
    ...
    for device in &devices {
        ...
        for i in 0..5 {
            match BitNetTensor::zeros(&[10], BitNetDType::F32, device, &pool) {
                ...
                Err(e) => {
                    println!("Rapid operation {} failed on device {}: {}",
                            i, bitnet_core::device::describe_device(device), e);
                }
            }
        }
    }
...
fn test_device_memory_pressure_handling() {
    ...
    for device in &devices {
        println!("Testing memory pressure on device: {}",
                bitnet_core::device::describe_device(device));
        ...
        for i in 0..max_attempts {
            ...
            match BitNetTensor::zeros(&[size], BitNetDType::F32, device, &pool) {
                Ok(tensor) => {
                    ...
                    if i % 10 == 0 {
                        let metrics = pool.get_metrics();
                        println!("  Iteration {}: {} allocations, {} bytes",
        ...
        let dtypes = vec![BitNetDType::F32, BitNetDType::F16, BitNetDType::I8, BitNetDType::I4];
        for &dtype in &dtypes {
        ...
        let final_metrics = pool.get_metrics();
        println!("  Final metrics: {} allocations, {} bytes",
...
fn test_concurrent_multi_device_operations() {
    ...
    for (device_idx, device) in devices.iter().enumerate() {
        ...
        let handle = thread::spawn(move || {
            ...
            println!("Thread {} starting on device: {}",
                    device_idx, bitnet_core::device::describe_device(&device_clone));
        ...
        let cross_device_handle = {
            ...
            thread::spawn(move || {
                ...
                for i in 0..10 {
                    ...
                    if let Ok(tensor1) = BitNetTensor::zeros(&[5, 5], BitNetDType::F32, &device1, &pool_clone) {
                        // Migrate to device 2
    ...
    let final_metrics = pool.get_metrics();
    println!("Final pool metrics: {} allocations, {} bytes",
            final_metrics.active_allocations, final_metrics.current_allocated);
...
fn test_large_scale_memory_stability() {
    ...
    for batch in 0..(iterations / batch_size) {
        ...
        if batch % 10 == 0 {
            ...
            let memory_growth = current_metrics.current_allocated as i64 - baseline_allocated as i64;
...
fn test_device_migration_memory_cleanup() {
    ...
    for i in 0..migration_count {
        ...
        if i % 5 == 0 {
            let current_metrics = pool.get_metrics();
            let memory_growth = current_metrics.current_allocated as i64 - initial_metrics.current_allocated as i64;
...
fn test_memory_tracking_integration() {
    ...
    for i in 0..tensor_count {
        ...
        if i % 10 == 0 {
            ...
            assert!(current_metrics.active_allocations >= initial_detailed_metrics.active_allocations);
            assert!(current_metrics.current_memory_usage >= initial_detailed_metrics.current_memory_usage);
            
            println!("Iteration {}: {} active allocations, {} bytes, pressure: {:?}",
                    i, current_metrics.active_allocations, current_metrics.current_memory_usage, current_pressure);
        }
    }
    ...
    assert!(final_metrics.active_allocations <= initial_detailed_metrics.active_allocations + 5,
           "Memory tracking shows potential leak: {} final vs {} initial allocations",
           final_metrics.active_allocations, initial_detailed_metrics.active_allocations);
    
    // Peak memory should be recorded
    assert!(final_metrics.peak_memory_usage >= peak_memory,
    ...
    assert!(!final_metrics.device_usage.is_empty(), "Device usage not tracked");
    
    // Check for memory pressure patterns if any were detected
    if !final_metrics.recent_patterns.is_empty() {
        println!("Detected {} memory patterns", final_metrics.recent_patterns.len());
        for pattern in &final_metrics.recent_patterns {
            ...
            if pattern.is_problematic {
                println!("Warning: Problematic pattern detected: {}", pattern.description);
            }
        }
    }
    ...
    F: FnOnce() -> R,
{
    ...
    let memory_used = final_metrics.current_allocated.saturating_sub(initial_metrics.current_allocated) as usize;
    let allocations_made = final_metrics.active_allocations.saturating_sub(initial_metrics.active_allocations) as usize;
    ...
    for device in &devices {
        ...
        for &dtype in &dtypes {
            for (shape, size_name) in &test_sizes {
                let operation_name = format!("{} {} {}", size_name, dtype, bitnet_core::device::describe_device(device));
                ...
                zeros_stats.assert_performance_threshold(Duration::from_millis(10), "Zeros creation");
                ...
                if dtype == BitNetDType::F32 {
                    ...
                    from_data_stats.assert_performance_threshold(Duration::from_millis(20), "From data creation");
                }
            }
        }
    }
    ...
    for shape in &test_shapes {
        ...
        let transpose_stats = run_benchmark(
            ...
            || {
                ...
                let _transposed = bitnet_core::tensor::transpose(&candle_tensor, &dims)
        ...
        let reshape_stats = run_benchmark(
            ...
            50,
            || {
                let _reshaped = tensor1.reshape(&[element_count])
        ...
        multiplication_stats.assert_performance_threshold(Duration::from_millis(100), "Multiplication");
...
fn benchmark_size_scaling_performance() {
    ...
    for (shape, size_name) in &size_tests {
        ...
        println!("Size {}: {} elements, creation time: {:?}, memory used: {} bytes, efficiency: {:.2}x",
    ...
    let dtypes_to_test = vec![BitNetDType::F32, BitNetDType::F16, BitNetDType::I8, BitNetDType::I4];
    ...
    for &dtype in &dtypes_to_test {
        ...
        println!("Dtype {}: avg time {:?}, expected size {} bytes, efficiency {:.1}x",
                dtype, dtype_stats.avg_duration, expected_bytes, dtype.memory_efficiency());
    }
}
...
fn benchmark_device_specific_performance() {
    ...
    for shape in &test_shapes {
        ...
        for device in &devices {
            ...
            let migration_stats = run_benchmark(
                ...
                10,
                || {
                    let _migrated = source_tensor.to_device(device, &pool)
                        .expect("Failed to migrate tensor");
                },
            );
            
            device_performance.push((device_name.clone(), creation_stats.avg_duration, migration_stats.avg_duration));
            
            println!("Device {}: creation {:?}, migration {:?}",
    ...
    if devices.len() >= 2 {
        ...
        match &candle1 + &candle2 {
            Ok(_) => {
                let cross_device_stats = run_benchmark(
                    "cross_device_addition",
                    3,
                    20,
                    || {
...
fn benchmark_concurrent_operation_performance() {
    ...
    for &thread_count in &concurrency_levels {
        ...
        println!("Concurrency {}: {} ops in {:?}, throughput: {:.2} ops/sec, avg per-op: {:?}",
        ...
        println!("Handle sharing {}: avg time {:?}, ops/sec: {:.2}",
...
fn benchmark_memory_efficiency() {
    ...
    for (shape, size_name) in &allocation_sizes {
        ...
        alloc_dealloc_stats.assert_performance_threshold(Duration::from_millis(20), "Allocation/deallocation");
    ...
    println!("Memory cleanup: {} bytes recovered, {:.1}% efficiency", 
             memory_recovered, cleanup_efficiency * 100.0);
    
    // Verify reasonable cleanup (at least 50% of allocated memory should be recovered, or no memory was allocated)
    assert!(cleanup_efficiency >= 0.5 || peak_metrics.current_allocated == 0,
           "Poor memory cleanup efficiency: {:.1}%", cleanup_efficiency * 100.0);
...
fn benchmark_quantization_performance() {
    ...
    for shape in &test_shapes {
        ...
        for &qtype in &quantized_types {
            ...
            println!("  {}: avg time {:?}, {} bytes, {:.1}x efficiency",
                    qtype, creation_stats.avg_duration, expected_bytes, memory_efficiency);
            
            // Quantized types should be fast to create
            creation_stats.assert_performance_threshold(Duration::from_millis(15), "Quantized creation");
        }
        ...
        println!("  BitNet 1.58b: avg time {:?}, 2 bits per element", bitnet158_stats.avg_duration);
    ...
    for &target_type in &quantized_types {
        ...
        println!("  Quantization to {}: avg time {:?}, {}x compression",
                target_type, conversion_stats.avg_duration, compression_ratio);
        
        // Quantization should be reasonably fast
        conversion_stats.assert_performance_threshold(Duration::from_millis(50), "Quantization conversion");
    }
    ...
    let quantized_arithmetic_stats = run_benchmark(
        "quantized_arithmetic",
        5,
        100,
        || {
    ...
    println!("  Quantized arithmetic: avg time {:?}", quantized_arithmetic_stats.avg_duration);
    quantized_arithmetic_stats.assert_performance_threshold(Duration::from_millis(10), "Quantized arithmetic");
    ...
    let (bitnet_tensor, bitnet_memory, _) = measure_memory_usage(&pool, || {
        BitNetTensor::zeros(&efficiency_test_shape, BitNetDType::BitNet158, &device, &pool)
            .expect("Failed to create BitNet tensor")
    });
    ...
    assert!(bitnet_efficiency >= 5.0, "BitNet158 should provide at least 5x memory efficiency");
...
fn benchmark_regression_testing() {
    ...
    let small_creation_stats = run_benchmark(
        "regression_small_creation",
        10,
        100,
        || {
    ...
    let medium_creation_stats = run_benchmark(
        "regression_medium_creation",
        10,
        50,
        || {
    ...
    let large_creation_stats = run_benchmark(
        "regression_large_creation",
        5,
        20,
        || {
    ...
    let arithmetic_stats = run_benchmark(
        "regression_arithmetic",
        5,
        50,
        || {
    ...
    let allocation_stats = run_benchmark(
        "regression_allocation",
        5,
        100,
        || {
    ...
    if devices.len() >= 2 {
        ...
        let migration_tensor = BitNetTensor::zeros(&[100, 100], BitNetDType::F32, &devices[0], &pool)
        ...
        let migration_stats = run_benchmark(
            ...
            10,
            || {
                let _migrated = migration_tensor.to_device(&devices[1], &pool)
                    .expect("Failed to migrate tensor");
            },
        );
    ...
    let handle_stats = run_benchmark(
        "regression_handles",
        10,
        1000,
        || {
    ...
    let mean_duration = consistency_measurements.iter().sum::<Duration>() / consistency_measurements.len() as u32;
    let variance: f64 = consistency_measurements.iter()
        .map(|d| {
            let diff = d.as_secs_f64() - mean_duration.as_secs_f64();
            diff * diff
        })
        .sum::<f64>() / consistency_measurements.len() as f64;
    let std_dev = variance.sqrt();
    let coefficient_of_variation = std_dev / mean_duration.as_secs_f64();
    
    println!("Performance consistency: mean {:?}, std dev {:.6}s, CV {:.2}%",
             mean_duration, std_dev, coefficient_of_variation * 100.0);
    
    // Verify consistency (CV should be less than 20% for stable performance)
    assert!(coefficient_of_variation < 0.2,
           "Performance too inconsistent: CV {:.2}%", coefficient_of_variation * 100.0);
    ...
    for (test_name, actual_duration) in &regression_results {
        if let Some((_, expected_duration)) = baseline_expectations.iter()
...
fn benchmark_device_migration_performance() {
    ...
    for (shape, size_name) in &test_cases {
        ...
        println!("  {} migration: avg {:?}", size_name, migration_duration / 100);
        ...
        for thread_id in 0..thread_count {
            ...
            let handle = thread::spawn(move || {
                for i in 0..operations_per_thread {
                    ...
                    let _tensor = BitNetTensor::zeros(&shape, BitNetDType::F32, &device_clone, &pool_clone)
                        .expect("Failed to create tensor in thread");
                }
            });
...
fn benchmark_focused_size_scaling_performance() {
    ...
    for &dtype in &dtypes_to_test {
        ...
        let expected_bytes = dtype.bytes_for_elements(10000);
        println!("  {}: avg time {:?}, {} bytes, {:.1}x efficiency",
                dtype, avg_duration, expected_bytes, dtype.memory_efficiency());
