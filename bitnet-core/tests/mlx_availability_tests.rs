...
fn test_mlx_device_manager() {
    use bitnet_core::{get_mlx_device_manager, MlxDeviceManager};
    ...
    match manager_result {
        Ok(manager) => {
            ...
            if has_gpu {
                ...
                assert!(
                    gpu_device.is_some(),
                    "Should have GPU device if has_gpu is true"
                );
                ...
                if let Some(gpu_dev) = gpu_device {
                    ...
                    assert!(
                        gpu_dev.supports_unified_memory,
                        "GPU should support unified memory"
                    );
                }
            }
    ...
    assert!(
        !devices.is_empty(),
        "Global manager should have at least CPU device"
    );
...
fn test_mlx_tensor_operations() {
    use bitnet_core::{
        create_mlx_array, default_mlx_device, is_mlx_available, BitNetMlxDevice, BitNetMlxTensor,
    };
...
fn test_mlx_operations() {
    use bitnet_core::{
        create_mlx_array, is_mlx_available, mlx_dequantize, mlx_matmul, mlx_quantize,
    };
...
fn test_mlx_bitnet_operations() {
    use bitnet_core::{
        create_mlx_array, default_mlx_device, is_mlx_available, BitNetMlxTensor, MlxOperations,
    };
...
fn test_mlx_candle_interoperability() {
    ...
    use bitnet_core::{
        candle_to_mlx_array, create_mlx_array, is_mlx_available, mlx_to_candle_tensor,
    };
                ...
                match candle_tensor2_result {
                    Ok(candle_tensor2) => {
                        ...
                        if let Ok(final_data) = candle_tensor2.flatten_all()?.to_vec1::<f32>() {
                            ...
                            let data_matches = final_data
                                .iter()
                                .zip(original_data.iter())
                                .all(|(a, b)| (a - b).abs() < 1e-6);
                            assert!(
                                data_matches,
                                "Data should be preserved in round-trip conversion"
                            );
                        }
                    }
                    Err(e) => {
...
fn test_mlx_feature_flag_behavior() {
    #[cfg(feature = "mlx")]
    {
        use bitnet_core::{default_mlx_device, get_mlx_device_manager, is_mlx_available};
...
fn test_platform_specific_mlx_behavior() {
    ...
    {
        use bitnet_core::{default_mlx_device, is_mlx_available};
        ...
        if mlx_available {
            ...
            match device_result {
                Ok(device) => {
                    println!(
                        "MLX device created on Apple Silicon: {}",
                        device.device_type()
                    );
...
fn test_mlx_device_selection_scenarios() {
    use bitnet_core::{auto_select_mlx_device, is_mlx_available, BitNetMlxDevice};
...
fn test_mlx_error_handling() {
    use bitnet_core::{create_mlx_array, is_mlx_available, mlx_matmul};
    ...
    assert!(
        ...
        "Should fail with mismatched data length"
    );
...
fn test_mlx_performance_benchmark() {
    use bitnet_core::{create_mlx_array, is_mlx_available, mlx_matmul, mlx_quantize};
    ...
    for &size in &sizes {
        ...
        if let (Ok(array_a), Ok(array_b)) = (array_a_result, array_b_result) {
            ...
            println!(
                "  Array creation failed, skipping benchmarks for size {}",
                size
            );
        }
    }
...
fn test_mlx_integration_comprehensive() {
    use bitnet_core::{
        candle_to_mlx_array, create_mlx_array, default_mlx_device, get_mlx_device_manager,
        is_mlx_available, mlx_to_candle_tensor, BitNetMlxTensor, MlxOperations,
    };
    ...
    for device_info in manager.available_devices() {
        ...
        println!(
            "✓ Device {} capabilities: {}",
            device_info.to_bitnet_device_type(),
            capabilities
        );
    }
...
fn test_mlx_vs_metal_vs_cpu_comparison() {
    ...
    #[cfg(feature = "mlx")]
    {
        use bitnet_core::mlx::{default_mlx_device, is_mlx_available};
