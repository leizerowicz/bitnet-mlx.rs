...
mod metal_shader_tests {
    ...
    fn test_shader_compiler_creation_and_config() {
        ...
        if let Ok(device) = device_result {
            ...
            assert_eq!(default_config.shader_directory, PathBuf::from("src/metal/shaders"));
            ...
            assert!(compiler_result.is_ok(), "Failed to create shader compiler with custom config");
            
            if let Ok(compiler) = compiler_result {
                let stats = compiler.get_stats();
                println!("Shader compiler stats: {:?}", stats);
                assert_eq!(stats.shader_directory, PathBuf::from("bitnet-core/src/metal/shaders"));
            }
        } else {
    ...
    fn test_individual_shader_file_compilation() {
        ...
        if let Ok(device) = device_result {
                    ...
                    match result {
                        Ok(compiled_shader) => {
                            ...
                            assert!(!compiled_shader.function_names.is_empty(), 
                                   "Shader {} should have at least one function", shader_file);
            ...
            println!("Successfully compiled {} shader files with {} total functions", 
                    compiled_count, total_functions);
            assert!(compiled_count > 0, "Should have compiled at least one shader file");
            assert!(total_functions > 0, "Should have found at least one shader function");
        } else {
    ...
    fn test_shader_function_discovery() {
        ...
        if let Ok(device) = device_result {
            ...
            let expected_functions = vec![
                // BitLinear functions
                ("bitlinear", vec!["bitlinear_forward", "bitlinear_backward_input", "binarize_weights", "quantize_activations"]),
                // Quantization functions
                ("quantization", vec![
                    "quantize_weights_1bit", "quantize_activations_8bit", 
                    "dequantize_weights_1bit", "dequantize_activations_8bit",
                    "dynamic_quantize_activations", "quantize_gradients", "mixed_precision_matmul"
                ]),
                // Activation functions
                ("activation", vec![
                    "relu_forward", "relu_backward", "gelu_forward", "gelu_backward",
                    "swish_forward", "swish_backward", "sigmoid_forward", "sigmoid_backward",
                    "tanh_forward", "tanh_backward", "leaky_relu_forward", "leaky_relu_backward",
                    "softmax_forward", "softmax_backward", "layer_norm_forward", "fused_relu_dropout"
                ]),
            ];

            for (shader_name, expected_funcs) in expected_functions {
                let shader = compiled_shaders.iter()
                ...
                if let Some(shader) = shader {
                    ...
                    for expected_func in expected_funcs {
                        assert!(shader.function_names.contains(&expected_func.to_string()),
                               "Shader '{}' should contain function '{}'", shader_name, expected_func);
                    }
        ...
        if let Ok(device) = device_result {
            ...
            match shaders_result {
                Ok(shaders) => {
                    ...
                    println!("Pipeline creation results: {} successful, {} failed", 
                    ...
                    if successful_pipelines == 0 && failed_pipelines > 0 {
                        println!("⚠ No pipelines created successfully - shader files may be missing");
                    }
                }
                Err(e) => {
    ...
    fn test_shader_compilation_error_handling() {
        ...
        if let Ok(device) = device_result {
            ...
            assert!(result.is_err(), "Should fail to compile non-existent shader file");
            ...
            assert!(result.is_err(), "Should fail to compile invalid source code");
            ...
            assert!(cached_shader.is_none(), "Should return None for non-existent cached shader");
            println!("✓ Correctly handled non-existent cached shader");

            // Test getting non-existent compute function
            let function_result = compiler.get_compute_function("non_existent_shader", "non_existent_function");
            assert!(function_result.is_err(), "Should fail to get non-existent compute function");
    ...
    fn test_shader_caching() {
        ...
        if let Ok(device) = device_result {
            ...
            if shader_path.exists() {
                ...
                assert!(stats_after_first.cached_shaders > 0, "Should have cached shaders after compilation");
                ...
                assert_eq!(stats_after_clear.cached_shaders, 0, "Cache should be empty after clearing");
    ...
    fn test_shader_compilation_configurations() {
        ...
        if let Ok(device) = device_result {
            ...
            for opt_level in optimization_levels {
                ...
                assert!(compiler_result.is_ok(), "Should create compiler with optimization level {:?}", opt_level);
                println!("✓ Compiler created with optimization level: {:?}", opt_level);
            }
            ...
            for lang_version in language_versions {
                ...
                let compiler_result = ShaderCompiler::new(device.clone(), config);
                assert!(compiler_result.is_ok(), "Should create compiler with language version {:?}", lang_version);
                println!("✓ Compiler created with language version: {:?}", lang_version);
            }
        } else {
    ...
    fn test_shader_function_parameter_validation() {
        ...
        let test_functions = vec![
            (BitNetShaderFunction::BitLinearForward, "bitlinear", "bitlinear_forward"),
            (BitNetShaderFunction::QuantizeWeights1Bit, "quantization", "quantize_weights_1bit"),
            (BitNetShaderFunction::ReluForward, "activation", "relu_forward"),
            (BitNetShaderFunction::GeluBackward, "activation", "gelu_backward"),
            (BitNetShaderFunction::MixedPrecisionMatmul, "quantization", "mixed_precision_matmul"),
        ];
    ...
    fn test_shader_compilation_performance() {
        ...
        if let Ok(device) = device_result {
            ...
            if compilation_time > Duration::from_millis(100) {
                assert!(cached_compilation_time < compilation_time, 
                ...
                Ok(mut loader) => {
                    ...
                    match preload_all_result {
                        Ok(()) => {
                            ...
                            let available_shaders = loader.get_available_shaders();
                            println!("Available shaders after preload all: {:?}", available_shaders);
                            ...
                            for shader_name in &available_shaders {
                                ...
                                match functions_result {
                                    Ok(functions) => {
                                        ...
                                        assert!(!functions.is_empty(), "Shader should have at least one function");
                                    }
                                    Err(e) => {
                                        println!("Failed to get functions for {}: {}", shader_name, e);
                                    }
                                }
                            }
                        }
                        Err(e) => {
                            println!("Preload all failed (expected if shader files missing): {}", e);
                        }
                    }
                }
                Err(e) => {
                    println!("Shader loader creation failed (expected if shader files missing): {}", e);
                }
            }
        } else {
                    ...
                    for data_size in test_data_sizes {
                        ...
                        match dispatch_result {
                            Ok((threadgroup_size, threadgroups)) => {
                                println!("Data size {}: threadgroup={:?}, threadgroups={:?}", 
                            ...
                            Err(e) => {
                                println!("Dispatch calculation failed for size {}: {}", data_size, e);
                            }
                        }
                    }
