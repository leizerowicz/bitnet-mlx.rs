...
use bitnet_core::tensor::{BitNetTensor, BitNetDType, BroadcastCompatible};
use bitnet_core::memory_integration::set_global_memory_pool;
...
fn test_tensor_memory_pressure_handling() {
    ...
    assert!(!tensors.is_empty(), "Should have created at least one tensor");
}
...
fn test_tensor_device_migration() {
    ...
    // Check device type matches (as comparison, since Device doesn't implement PartialEq)
    match (tensor.device(), &cpu_device) {
        (dev1, dev2) => {
            // Both should be CPU devices
            assert!(matches!(dev1, bitnet_core::Device::Cpu));
            assert!(matches!(dev2, bitnet_core::Device::Cpu));
        }
    }
    
    // Test device migration
    let migrated_tensor = tensor.to_device(&cpu_device)
        .expect("Failed to migrate tensor to CPU");
        
    // Check device type matches (as comparison, since Device doesn't implement PartialEq)
    match (migrated_tensor.device(), &cpu_device) {
        (dev1, dev2) => {
            // Both should be CPU devices
            assert!(matches!(dev1, bitnet_core::Device::Cpu));
            assert!(matches!(dev2, bitnet_core::Device::Cpu));
...
fn test_tensor_shape_operations() {
    ...
    // Test squeeze
    let tensor_with_ones = BitNetTensor::zeros(&[1, 3, 1, 4], BitNetDType::F32, Some(device.clone()))
...
fn test_tensor_broadcasting_compatibility() {
    ...
    // Test incompatible shapes - [3, 1] cannot broadcast with [2, 5] because 3 != 2
...
fn test_tensor_thread_safety() {
    ...
    let shared_tensor = Arc::new(BitNetTensor::zeros(&[10, 10], BitNetDType::F32, Some(device))
...
fn test_tensor_concurrent_operations() {
    ...
    let tensors: Vec<_> = handles.into_iter()
...
fn test_tensor_error_handling() {
    ...
    assert!(result.is_err(), "Reshape with different element count should fail");
