...
use bitnet_core::device::{get_metal_device, get_cpu_device, auto_select_device, is_metal_available, get_metal_device_name};
...
fn test_cpu_device_always_works() {
    let cpu_device = get_cpu_device();
    let tensor = Tensor::ones(&[3, 3], DType::F32, &cpu_device).expect("Failed to create tensor on CPU");
...
fn test_metal_compatibility_functions() {
    ...
    if !metal_available {
        ...
        assert!(!has_device_name, "Device name should be None when Metal is not available");
    }
}
