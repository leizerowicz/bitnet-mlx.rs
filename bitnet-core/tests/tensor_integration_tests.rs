...
impl TensorTestEnvironment {
    fn new(config: IntegrationTestConfig) -> Self {
        ...
        let pool = Arc::new(
            HybridMemoryPool::with_config(pool_config)
        ...
        let cleanup_manager = if config.enable_cleanup {
            ...
            Some(CleanupManager::new(cleanup_config, pool.clone())
    ...
    fn get_metrics_summary(&self) -> MetricsSummary {
        ...
        let pressure_events = history.iter()
...
fn test_end_to_end_tensor_lifecycle_workflow() {
    ...
    let pressure_tensors = create_test_tensor_batch(&env, 50, &[256, 256], BitNetDType::F32, device);
    env.record_metrics();
...
fn test_cross_system_integration_validation() {
    ...
    let test_scenarios = vec![
        ...
        ("quantized_efficient", 50, vec![64, 64], BitNetDType::BitNet158),
    ];
    ...
    if env.devices.len() > 1 {
        ...
        let cross_device_tensor = BitNetTensor::zeros(&[256, 256], BitNetDType::F32, source_device, &env.pool)
            .expect("Failed to create cross-device tensor");
        
        let migrated_tensor = cross_device_tensor.to_device(target_device, &env.pool)
