...
use bitnet_core::tokenizer::{
    add_special_tokens, create_bpe_tokenizer, create_simple_tokenizer, decode_tokens, encode_text,
    get_special_token_id, Tokenizer,
};
...
fn test_special_token_numeric_ids() {
    ...
    let tokens_with_extreme_ids = vec![("[MIN]", 0), ("[MAX]", u32::MAX), ("[MID]", u32::MAX / 2)];
    ...
    assert_eq!(
        get_special_token_id(&tokenizer, "[MID]"),
        Some(u32::MAX / 2)
    );
}
...
fn test_special_tokens_with_bpe_tokenizer() {
    ...
    let special_tokens = vec![("[CLS]", 100), ("[SEP]", 101), ("[MASK]", 102)];
...
fn test_special_tokens_persistence() {
    ...
    // Add special tokens
    let original_tokens = vec![("[CLS]", 100), ("[SEP]", 101), ("[PAD]", 102)];
...
fn test_special_tokens_concurrent_access() {
    ...
    for i in 0..10 {
        let token_name = format!("[THREAD_{i}]");
        assert_eq!(
            get_special_token_id(&tok, &token_name),
            Some(i as u32 + 2000)
        );
    }
}
...
fn test_special_tokens_boundary_conditions() {
    ...
    assert_eq!(
        get_special_token_id(&tokenizer, &very_long_token),
        Some(999)
    );
    ...
    assert_eq!(
        get_special_token_id(&tokenizer, "[TOKEN\u{0000}]"),
        Some(1000)
    );
    assert_eq!(
        get_special_token_id(&tokenizer, "[TOKEN\u{FFFF}]"),
        Some(1001)
    );
}
...
fn test_special_tokens_performance() {
    ...
    let token_refs: Vec<(&str, u32)> = tokens.iter().map(|(s, id)| (s.as_str(), *id)).collect();
    ...
    assert_eq!(
        get_special_token_id(&tokenizer, "[PERF_TOKEN_9999]"),
        Some(9999)
    );
}
