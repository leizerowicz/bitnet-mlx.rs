...
use bitnet_core::memory::{
    conversion::{ConversionConfig, ConversionEngine, ConversionQuality},
...
fn test_bitnet_tensor_to_array_data() {
    ...
    let extracted_data = candle_tensor
        .flatten_all()
        .unwrap()
        .to_vec1::<f32>()
        .unwrap();
...
fn test_array_conversion_strategies() {
    ...
    // Test strategy info
    let strategy_info =
        engine.get_optimal_strategy_info(BitNetDType::F32, BitNetDType::F16, &[16, 16], &device);
...
fn test_array_conversion_quality() {
    ...
    // Test fast quality
    let fast_result =
        engine.convert_with_quality(&tensor, BitNetDType::F16, ConversionQuality::Fast);
    assert!(fast_result.is_ok());

    // Test balanced quality
    let balanced_result =
        engine.convert_with_quality(&tensor, BitNetDType::F16, ConversionQuality::Balanced);
    assert!(balanced_result.is_ok());

    // Test precise quality
    let precise_result =
        engine.convert_with_quality(&tensor, BitNetDType::F16, ConversionQuality::Precise);
...
fn test_array_conversion_edge_cases() {
    ...
    let large_1d_tensor =
        BitNetTensor::from_data(large_1d_data, &[1000000], &device, &pool).unwrap();
...
mod mlx_array_tests {
    use super::*;
    use bitnet_core::mlx::{candle_to_mlx_array, create_mlx_array, mlx_to_candle_tensor};
