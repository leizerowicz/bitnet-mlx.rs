...
fn test_cpu_device_equality() {
    ...
    assert!(matches!(details.metadata.capability_match, CapabilityMatch::Identical));
}
...
fn test_auto_device_selection_comparison() {
    ...
    match auto_device {
        bitnet_core::Device::Cpu => {
            assert!(comparison.is_equal());
            assert!(matches!(comparison.details().metadata.capability_match, CapabilityMatch::Identical));
        }
        bitnet_core::Device::Metal(_) => {
            assert!(comparison.is_not_equal());
            assert!(matches!(comparison.details().metadata.capability_match, CapabilityMatch::Compatible));
            assert!(comparison.details().metadata.unified_memory_compatible);
        }
        bitnet_core::Device::Cuda(_) => {
            assert!(comparison.is_not_equal());
            assert!(matches!(comparison.details().metadata.capability_match, CapabilityMatch::Incompatible));
            assert!(!comparison.details().metadata.unified_memory_compatible);
        }
    }
}
...
fn test_metal_device_comparison() {
    if let Ok(metal1) = get_metal_device() {
        if let Ok(metal2) = get_metal_device() {
            ...
            assert!(comparison.is_equal());
            assert!(matches!(comparison.details().device1_type, DeviceType::Metal { .. }));
            assert!(matches!(comparison.details().device2_type, DeviceType::Metal { .. }));
...
fn test_cpu_metal_cross_comparison() {
    ...
    if let Ok(metal) = get_metal_device() {
        ...
        assert!(matches!(reverse_comparison.details().device1_type, DeviceType::Metal { .. }));
        assert!(matches!(reverse_comparison.details().device2_type, DeviceType::Cpu));
    }
}
...
fn test_capability_match_descriptions() {
    assert_eq!(CapabilityMatch::Identical.description(), "identical capabilities");
    assert_eq!(CapabilityMatch::Compatible.description(), "compatible capabilities");
    assert_eq!(CapabilityMatch::Incompatible.description(), "incompatible capabilities");
    assert_eq!(CapabilityMatch::Unknown.description(), "unknown capability compatibility");
}
...
fn test_device_comparison_metadata() {
    ...
    assert!(matches!(metadata.capability_match, CapabilityMatch::Identical));
}
...
fn test_metal_device_metadata() {
    if let Ok(metal) = get_metal_device() {
        ...
        assert!(!metadata.same_hardware);
        assert!(metadata.unified_memory_compatible);
        assert!(matches!(metadata.capability_match, CapabilityMatch::Compatible));
    }
}
...
fn test_device_comparison_consistency() {
    ...
    assert_eq!(comparison1.details().metadata.capability_match, 
...
fn test_metal_device_instances_equality() {
    ...
    let metal_devices: Vec<_> = metal_results.iter().filter_map(|r| r.as_ref().ok()).collect();
...
fn test_device_comparison_memory_usage() {
    ...
    let comparisons: Vec<_> = (0..100).map(|_| {
        compare_devices(&cpu, &auto_device)
    }).collect();
