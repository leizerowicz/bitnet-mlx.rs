...
use bitnet_core::tensor::acceleration::{
    AccelerationBackend, AccelerationBackendImpl, DispatchStrategy, OperationContext,
    OperationType, PerformanceRequirements, SimdAccelerator, SimdOptimization,
};
...
mod tests {
    ...
    fn setup_test_environment() {
        ...
        let pool = POOL_HOLDER.get_or_init(|| Arc::new(HybridMemoryPool::new().unwrap()));
    ...
    fn test_simd_accelerator_creation() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(
            !accelerator.is_available()
                || accelerator.optimization_level() != SimdOptimization::None
        );
        ...
        println!(
            "SIMD Accelerator created with optimization level: {:?}",
            accelerator.optimization_level()
        );
    ...
    fn test_acceleration_backend_priorities() {
        ...
        for backend in &backends {
            ...
            println!(
                "{} performance: {:.1} GFLOPS, {:.1} μs latency, {:.1} GB/s bandwidth",
        ...
        for op in &operations {
            ...
            match op {
                OperationType::MatMul | OperationType::Convolution => {
                    ...
                    assert!(
                        preferred == AccelerationBackend::MLX
                            || preferred == AccelerationBackend::Metal
                    );
                }
    ...
    fn test_operation_context() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let input_elements: usize = context
            .input_shapes
            .iter()
        ...
        assert!(
            (memory as i64 - estimated_memory as i64).abs() < estimated_memory as i64 / 10,
            "Memory estimation: expected ~{estimated_memory}, got {memory}"
        );

        println!(
            "Operation context: complexity={:.1}, memory={}MB",
            complexity,
            memory / (1024 * 1024)
        );
    ...
    fn test_operation_dispatcher_creation() -> Result<(), Box<dyn std::error::Error>> {
        ...
        match result {
            Ok(dispatcher) => {
                ...
                println!(
                    "Dispatcher created successfully with {} backends",
                    available.len()
                );
            }
            Err(e) => {
...
mod benchmark_tests {
    ...
    fn benchmark_simd_optimization_detection() {
        ...
        println!(
            "SIMD detection benchmark: {iterations} iterations, avg {avg_time}ns per detection"
        );
