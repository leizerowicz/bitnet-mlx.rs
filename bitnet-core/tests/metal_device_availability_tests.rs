...
use bitnet_core::device::{
    auto_select_device, describe_device, get_cpu_device, get_device_info, get_metal_device,
    get_metal_device_name, is_metal_available, DeviceError,
};
...
fn test_metal_rs_device_creation() {
    use bitnet_core::metal::{create_command_queue, create_library, create_metal_device};
...
fn test_metal_context_initialization() {
    use bitnet_core::metal::{create_buffer, initialize_metal_context, read_buffer};
...
fn test_auto_device_selection() {
    ...
    match device {
        Device::Metal(_) => {
            ...
            assert!(
                is_metal_available(),
                "Metal device selected but availability check failed"
            );
        }
        Device::Cpu => {
...
fn test_metal_device_creation_performance() {
    ...
    for i in 0..iterations {
        ...
        match result {
            Ok(_) => {
                ...
                println!(
                    "Iteration {}: Metal device created in {:?}",
                    i + 1,
                    duration
                );
            }
            Err(e) => {
                println!(
                    "Iteration {}: Metal device creation failed in {:?}: {}",
                    i + 1,
                    duration,
                    e
                );
    ...
    assert!(
        average_time < std::time::Duration::from_millis(100),
...
fn test_metal_integration_workflow() {
    use bitnet_core::metal::{
        create_buffer, create_metal_device, initialize_metal_context, read_buffer,
    };
...
fn test_metal_memory_management() {
    use bitnet_core::metal::{create_buffer, create_metal_device, read_buffer};
...
fn test_metal_device_memory_limits() {
    ...
    if let Ok(device) = create_metal_device() {
        ...
        let max_buffer_length = device.max_buffer_length();
        println!(
            "Max buffer length: {} bytes ({:.2} GB)",
            max_buffer_length,
            max_buffer_length as f64 / (1024.0 * 1024.0 * 1024.0)
        );
        assert!(max_buffer_length > 0);

        let max_threadgroup_memory = device.max_threadgroup_memory_length();
        println!(
            "Max threadgroup memory: {} bytes ({:.2} KB)",
            max_threadgroup_memory,
            max_threadgroup_memory as f64 / 1024.0
        );
        assert!(max_threadgroup_memory > 0);
        ...
        println!(
            "Recommended max working set: {} bytes ({:.2} GB)",
            recommended_max_working_set,
            recommended_max_working_set as f64 / (1024.0 * 1024.0 * 1024.0)
        );
        ...
        println!(
            "Testing texture size limits (assuming {}x{} is supported)",
        ...
        match library_result {
            Ok(library) => {
                ...
                for function_name in &function_names {
                    let pipeline_result =
                        create_compute_pipeline_with_library(&device, &library, function_name);
                    match pipeline_result {
                        Ok(pipeline) => {
                            println!(
                                "Successfully created pipeline for function: {}",
                                function_name
                            );
...
fn test_metal_command_buffer_operations() {
    use bitnet_core::metal::{create_buffer, create_command_queue, create_metal_device};
    ...
    if let Ok(device) = create_metal_device() {
        ...
        println!(
            "Command buffer committed, status: {:?}",
            command_buffer.status()
        );
        ...
        println!(
            "Command buffer completed, final status: {:?}",
            command_buffer.status()
        );
...
fn test_metal_buffer_pool_advanced() {
    use bitnet_core::metal::{
        create_buffer_pool_with_config, create_metal_device, BufferPoolConfig,
    };
    ...
    if let Ok(device) = create_metal_device() {
        ...
        println!(
            "Allocated {} buffers in {:?}",
            allocated_buffers.len(),
            allocation_time
        );
...
fn test_metal_synchronization_advanced() {
    use bitnet_core::metal::{create_command_queue, create_metal_device, create_synchronizer};
        ...
        for (i, mut sync_point) in sync_points.into_iter().enumerate() {
            ...
            match signal_result {
                Ok(()) => {
                    ...
                    let wait_result = synchronizer
                        .wait_for_event_timeout(&sync_point, Duration::from_millis(100));
                    match wait_result {
                        Ok(completed) => {
...
fn test_metal_error_handling_edge_cases() {
    use bitnet_core::metal::{create_buffer, create_metal_device, MetalError};
    ...
    if let Ok(device) = create_metal_device() {
        ...
        let huge_buffer_result =
            device.new_buffer(huge_size, metal::MTLResourceOptions::StorageModeShared);
        // Note: This might not fail on all systems, but we test the behavior
        println!(
            "Huge buffer creation result: length = {}",
            huge_buffer_result.length()
        );

        // Test invalid shader compilation
        let invalid_shader = "this is not valid metal code";
        let invalid_library_result =
            device.new_library_with_source(invalid_shader, &metal::CompileOptions::new());
...
fn test_metal_library_function_discovery() {
    use bitnet_core::metal::{create_library_from_source, create_metal_device};
    ...
    if let Ok(device) = create_metal_device() {
        ...
        println!(
            "Default library contains {} functions",
...
fn test_metal_integration_comprehensive() {
    use bitnet_core::metal::{
        create_buffer, create_buffer_pool, create_command_queue,
        create_compute_pipeline_with_library, create_library_from_source, create_metal_device,
