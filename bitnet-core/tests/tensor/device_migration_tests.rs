...
use bitnet_core::device::{
    ...
    Device, DeviceType, DeviceCapabilities
};
...
fn get_device_name(device: &candle_core::Device) -> String {
    match device {
        ...
        candle_core::Device::Metal(metal) => format!("Metal({})", metal.device_name().unwrap_or("Unknown".to_string())),
        candle_core::Device::Cuda(cuda) => format!("CUDA({})", cuda.id()),
        _ => "Unknown".to_string(),
    }
}
...
fn create_migration_pool() -> HybridMemoryPool {
    ...
    HybridMemoryPool::with_config(config)
        .expect("Failed to create migration test memory pool")
}
...
fn test_metal_device_availability() {
    ...
    let metal_device = get_metal_device()
        .expect("Metal should be available");
...
fn test_cpu_to_metal_migration_placeholder() {
    ...
    for shape in &config.test_shapes {
        ...
        println!("Would migrate {:?} tensor from {} to {}", 
               shape, 
        ...
        assert!(matches!(metal_tensor.device(), candle_core::Device::Metal(_)));
...
fn test_migration_performance_baseline() {
    ...
    for shape in &config.test_shapes {
        ...
        println!("Baseline creation {:?}: {} bytes in {:?} ({:.2} MB/s)",
    ...
    let avg_throughput: f64 = baseline_metrics.iter()
...
fn test_concurrent_device_operations() {
    ...
    let handles: Vec<_> = (0..num_threads).map(|thread_id| {
        ...
        thread::spawn(move || {
            ...
            println!("Thread {} created {} tensors", thread_id, thread_tensors.len());
            thread_tensors.len()
        })
    }).collect();
...
fn test_device_memory_characteristics() {
    ...
    let initial_usage = pool.get_detailed_metrics()
    ...
    let cpu_usage = pool.get_detailed_metrics()
    ...
    if is_metal_available() {
        let metal_device = get_metal_device().expect("Metal should be available");
        let pre_metal_usage = pool.get_detailed_metrics()
            .map(|m| m.current_memory_usage)
            .unwrap_or(0);
        ...
        let metal_usage = pool.get_detailed_metrics()
            .map(|m| m.current_memory_usage - pre_metal_usage)
            .unwrap_or(0);
        
        println!("Metal memory usage for {} tensors: {} bytes", 
               metal_tensors.len(), metal_usage);
    }
}
...
fn test_device_resource_cleanup() {
    ...
    println!("Initial usage: {} bytes", initial_metrics.current_memory_usage);
    println!("Final usage: {} bytes", final_metrics.current_memory_usage);
    
    // Memory should not grow indefinitely
    assert!(final_metrics.current_memory_usage <= initial_metrics.current_memory_usage + 1024*1024);
