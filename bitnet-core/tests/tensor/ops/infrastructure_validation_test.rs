...
use bitnet_core::memory::{HybridMemoryPool, HybridMemoryPoolConfig};
...
#[tokio::test]
async fn test_infrastructure_validation() -> Result<(), Box<dyn std::error::Error>> {
...
fn test_error_handling_infrastructure() {
    ...
    match error {
        TensorOpError::DTypeMismatch { expected, actual } => {
            ...
            assert_eq!(actual, BitNetDType::I32);
        }
