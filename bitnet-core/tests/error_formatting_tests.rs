...
use bitnet_core::error::{
    BitNetError, BitNetErrorKind, ErrorContext, ErrorContextBuilder, ErrorFormatter, ErrorSeverity,
    ToBitNetError,
};
...
fn test_memory_error_with_size_information() {
    ...
    assert_eq!(
        memory_info,
        Some((Some(1024 * 1024 * 1024), "gpu_allocation"))
    );
...
fn test_conversion_error_formatting() {
    let error = BitNetError::conversion_error("Lossy conversion detected", "f64", "i8")
    ...
    assert!(report
        .suggestions
        .iter()
        .any(|s| s.contains("intermediate conversion")));
...
fn test_mlx_error_formatting() {
    let error = BitNetError::mlx_error("MLX array operation failed", "matrix_multiplication")
    ...
    assert!(report
        .suggestions
        .iter()
        .any(|s| s.contains("Apple Silicon")));
...
fn test_configuration_error_with_parameter() {
    let error =
        BitNetError::config_error_with_param("Invalid batch size configuration", "batch_size")
...
fn test_io_error_with_path() {
    ...
    assert!(report
        .suggestions
        .iter()
        .any(|s| s.contains("/path/to/model.safetensors")));
}

#[test]
fn test_concurrency_error_formatting() {
    let error =
        BitNetError::concurrency_error("Deadlock detected in memory pool", "gpu_memory_pool")
    ...
    assert!(report
        .suggestions
        .iter()
        .any(|s| s.contains("gpu_memory_pool")));
...
fn test_internal_error_formatting() {
    let error =
        BitNetError::internal_error("Unexpected state in tensor lifecycle", "tensor_manager")
    ...
    assert!(report
        .suggestions
        .iter()
        .any(|s| s.contains("internal error")));
...
fn test_error_context_builder() {
    ...
    assert_eq!(
        context.description,
        Some("Creating large tensor for training".to_string())
    );
...
fn test_error_formatter_customization() {
    let error =
        BitNetError::device_error("Test error", "CPU").add_context("test_key", "test_value");
...
fn test_performance_metrics_in_error_report() {
    ...
    assert!(report
        .suggestions
        .iter()
        .any(|s| s.contains("batch size") || s.contains("dimensions")));
}
