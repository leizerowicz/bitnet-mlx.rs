    ...
    fn create_tensor(&self, shape: &[usize], dtype: BitNetDType, data: Option<&[f32]>) -> AccelerationResult<BitNetTensor> {
        match data {
            Some(_data) => {
                ...
                BitNetTensor::zeros(shape, dtype, Some(Device::Cpu)).map_err(|e| AccelerationError::OperationFailed {
                    backend: "SIMD".to_string(),
...
pub fn create_simd_accelerator() -> AccelerationResult<Option<Box<dyn AccelerationBackendImpl + Send + Sync>>> {
