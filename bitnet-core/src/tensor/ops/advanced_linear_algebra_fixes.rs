...
pub fn svd_with_memory_pool(
    ...
    #[cfg(feature = "tracing")]
    debug!(
        "Computing production SVD with memory pool for tensor: {:?}",
        input.shape().dims()
    );
...
pub fn qr_with_memory_pool(
    ...
    #[cfg(feature = "tracing")]
    debug!(
        "Computing production QR decomposition with memory pool for tensor: {:?}",
        input.shape().dims()
    );
...
pub fn cholesky_with_memory_pool(
    ...
    #[cfg(feature = "tracing")]
    debug!(
        "Computing production Cholesky decomposition with memory pool for tensor: {:?}",
        input.shape().dims()
    );
...
fn bidiagonalize_householder(
    matrix: &mut Vec<Vec<f32>>,
    m: usize,
    n: usize,
) -> TensorOpResult<(Vec<Vec<f32>>, Vec<Vec<f32>>)> {
    ...
    for i in 0..min_dim {
        ...
        if i < m - 1 {
            let (householder_left, _) = compute_householder_vector(
                &matrix[i..].iter().map(|row| row[i]).collect::<Vec<_>>(),
            )?;
...
fn bidiagonal_svd(
    ...
    _n: usize,
    min_dim: usize,
) -> TensorOpResult<(Vec<Vec<f32>>, Vec<f32>, Vec<Vec<f32>>)> {
...
fn accumulate_u_transformation(
    u_accum: &mut Vec<Vec<f32>>,
    householder: &[f32],
    start_row: usize,
    m: usize,
) {
...
fn accumulate_v_transformation(
    v_accum: &mut Vec<Vec<f32>>,
    householder: &[f32],
    start_col: usize,
    n: usize,
) {
