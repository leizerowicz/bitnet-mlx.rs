...
pub fn power_iteration(
    ...
    for iteration in 0..max_iterations {
        ...
        // Normalize v_new
        let norm_squared = (&v_new_candle * &v_new_candle)?.sum_all()?.to_scalar::<f32>()?;
...
fn jacobi_eigendecomposition(
    ...
    for iteration in 0..max_iterations {
        ...
        #[cfg(feature = "tracing")]
        if iteration % 1000 == 0 {
            trace!("Jacobi iteration {}: max off-diagonal = {:.2e}", iteration, max_val);
        }
    }
...
pub fn inverse_power_iteration(
    ...
    for iteration in 0..max_iterations {
        ...
        let numerator = (&v_candle * &av_new_candle)?.sum_all()?.to_scalar::<f32>()?;
        let denominator = (&v_new_candle * &v_new_candle)?.sum_all()?.to_scalar::<f32>()?;
        ...
        if iteration > 0 && (new_eigenvalue - eigenvalue).abs() < tolerance {
            #[cfg(feature = "tracing")]
            debug!("Inverse power iteration converged after {} iterations", iteration + 1);
            return Ok((new_eigenvalue, v));
        }
...
fn extract_diagonal(matrix: &BitNetTensor) -> TensorOpResult<BitNetTensor> {
    ...
    BitNetTensor::from_vec(diagonal_data, &[n], matrix.dtype(), Some(matrix.device().clone()))
        .map_err(|e| TensorOpError::InternalError {
...
fn solve_linear_system(a: &BitNetTensor, b: &BitNetTensor) -> TensorOpResult<BitNetTensor> {
    ...
    BitNetTensor::from_candle(result, a.device())
        .map_err(|e| TensorOpError::InternalError {
...
fn construct_upper_triangular(data: &[f32], n: usize) -> TensorOpResult<CandleTensor> {
    ...
    CandleTensor::from_slice(data, (n, n), &device)
        .map_err(|e| TensorOpError::CandleError {
...
fn validate_symmetric_matrix(tensor: &BitNetTensor) -> TensorOpResult<()> {
    ...
    for i in 0..n.min(10) { // Check first 10x10 for efficiency
        for j in 0..i {
