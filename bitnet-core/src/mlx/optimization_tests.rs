...
mod tests {
    ...
    use super::super::{MlxTensor, BitNetMlxDevice};
    ...
    fn test_memory_optimizer_pool_operations() {
        ...
        let array1 = optimizer.get_or_create_tensor(&shape, dtype, &device).unwrap();
        let stats = optimizer.get_stats();
        ...
        let _array2 = optimizer.get_or_create_tensor(&shape, dtype, &device).unwrap();
        let stats = optimizer.get_stats();
    ...
    fn test_memory_optimizer_clear_pool() {
        ...
        let array = optimizer.get_or_create_tensor(&shape, dtype, &device).unwrap();
    ...
    fn test_auto_tuner_benchmarking() {
        ...
        let best_config = tuner.benchmark_operation("test_op", configs, benchmark_fn).unwrap();
    ...
    fn test_batch_optimizer_find_optimal_size() {
        ...
        let optimal_size = optimizer.find_optimal_batch_size("test_op", 64, benchmark_fn).unwrap();
    ...
    fn test_batch_optimizer_process_in_batches() {
        ...
        let results = optimizer.process_in_batches("test_op", inputs, process_fn).unwrap();
