...
impl Tokenizer {
    ...
    pub fn decode(&self, ids: &[u32]) -> Result<String> {
        match &self.inner {
            #[cfg(feature = "tokenizers")]
            TokenizerType::HuggingFace(tokenizer) => tokenizer
                .decode(ids, false)
...
pub fn load_hf_tokenizer(_model_id: &str) -> Result<Tokenizer> {
    #[cfg(feature = "tokenizers")]
    {
        ...
        let hf_tokenizer = HfTokenizer::from_file(&tokenizer_path).map_err(|e| {
            anyhow::anyhow!(
                "Failed to load HuggingFace tokenizer from '{}': {}",
                tokenizer_path,
                e
            )
        })?;
        ...
        for word in words {
            if let Some(&token_id) = self.vocab.get(word) {
                ...
                if let Some(&unk_id) = self.vocab.get("<unk>") {
                    ...
                    return Err(anyhow::anyhow!(
                        "Unknown token '{}' and no <unk> token found",
                        word
                    ));
                }
            }
...
mod tests {
    ...
    fn test_special_tokens_standalone_functions() {
        ...
        let special_tokens = vec![("[START]", 200), ("[END]", 201)];
