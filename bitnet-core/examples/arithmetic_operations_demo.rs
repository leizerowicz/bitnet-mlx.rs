...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    bitnet_core::tensor::memory_integration::set_global_memory_pool(std::sync::Arc::downgrade(&memory_pool));
    ...
    println!("  Broadcast [3,4] + [1,4]: shape {:?}", broadcast_result.shape().dims());
    
    // Broadcasting with scalar: [3, 4] + [1, 1] -> [3, 4]
    let scalar_broadcast = add(&tensor_2d, &tensor_scalar)?;
    println!("  Broadcast [3,4] + [1,1]: shape {:?}", scalar_broadcast.shape().dims());
    ...
    println!("  Complex broadcast [2,3,1] + [1,1,4]: shape {:?}", complex_broadcast.shape().dims());
    ...
    println!("  After in-place addition: shape {:?}", mutable_tensor.shape().dims());
    
    // In-place subtraction
    sub_(&mut mutable_tensor, &operand)?;
    println!("  After in-place subtraction: shape {:?}", mutable_tensor.shape().dims());
    
    // In-place multiplication
    mul_(&mut mutable_tensor, &operand)?;
    println!("  After in-place multiplication: shape {:?}", mutable_tensor.shape().dims());
    
    // In-place division
    div_(&mut mutable_tensor, &operand)?;
    println!("  After in-place division: shape {:?}", mutable_tensor.shape().dims());
    ...
    println!("  Add scalar 5.0: shape {:?}", scalar_add_result.shape().dims());
    
    // Scalar subtraction
    let scalar_sub_result = sub_scalar(&base_tensor, 2.0)?;
    println!("  Subtract scalar 2.0: shape {:?}", scalar_sub_result.shape().dims());
    
    // Scalar multiplication
    let scalar_mul_result = mul_scalar(&base_tensor, 3.0)?;
    println!("  Multiply by scalar 3.0: shape {:?}", scalar_mul_result.shape().dims());
    
    // Scalar division
    let scalar_div_result = div_scalar(&base_tensor, 2.0)?;
    println!("  Divide by scalar 2.0: shape {:?}", scalar_div_result.shape().dims());
    ...
    println!("  After adding 10.0 in-place: shape {:?}", scalar_tensor.shape().dims());
    
    mul_scalar_(&mut scalar_tensor, 0.5)?;
    println!("  After multiplying by 0.5 in-place: shape {:?}", scalar_tensor.shape().dims());
    ...
    println!("  F32 addition: {:?} -> {:?}", f32_a.dtype(), f32_result.shape().dims());
    ...
    println!("  I32 addition: {:?} -> {:?}", i32_a.dtype(), i32_result.shape().dims());
