...
fn demo_matrix_multiplication() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Matrix A: {:?} × Matrix B: {:?}", a_large.shape().dims(), b_large.shape().dims());
    ...
    println!("   Batch A: {:?} × Batch B: {:?}", a_batch.shape().dims(), b_batch.shape().dims());
...
fn demo_dot_products() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Vector X: {:?}, Vector Y: {:?}", x.shape().dims(), y.shape().dims());
    ...
    println!("   Vector U: {:?}, Vector V: {:?}", u.shape().dims(), v.shape().dims());
    ...
    println!("   Tensor A: {:?}, Tensor B: {:?}", a.shape().dims(), b.shape().dims());
...
fn demo_matrix_transformations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for size in sizes {
        ...
        println!("   {}×{} identity matrix created in {:?} with shape {:?}", 
                 size, size, duration, identity.shape().dims());
    }
...
fn demo_optimization_strategies() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (size, _, description) in matrix_sizes {
        println!("\n{}. {} ({}×{}):", 
            if size == 64 { 1 } else if size == 256 { 2 } else { 3 },
