...
fn run_graph_demo() -> Result<(), Box<dyn std::error::Error>> {
    use bitnet_core::mlx::{GraphBuilder, Operation, FusionPattern};
    ...
    println!("Topological sort successful: {} nodes", execution_order.len());
    
    for (i, &node_id) in execution_order.iter().enumerate() {
        if let Some(node) = graph.get_node(node_id) {
            println!("  Step {}: {} (shape: {:?})", i + 1, node.operation, node.shape);
        }
    }
    ...
    for (i, opportunity) in fusion_opportunities.iter().enumerate() {
        ...
        println!("    Estimated speedup: {:.2}x", opportunity.estimated_speedup);
    ...
    for (i, group) in memory_plan.memory_groups.iter().enumerate() {
        println!("  Group {}: {} tensors can share memory", i + 1, group.len());
    ...
    println!("  Execution steps: {}", execution_plan.execution_order.len());
    println!("  Fusion opportunities: {}", execution_plan.fusion_opportunities.len());
    println!("  Memory groups: {}", execution_plan.memory_plan.memory_groups.len());
    println!("  Estimated memory usage: {} bytes ({:.2} MB)", 
    ...
    let total_speedup: f32 = execution_plan.fusion_opportunities.iter()
        .map(|op| op.estimated_speedup)
        .product();
    
    if total_speedup > 1.0 {
        println!("  Potential speedup from fusion: {:.2}x", total_speedup);
        println!("  Optimized execution time: {:.6} seconds", 
    ...
    println!("  Peak memory usage: {} bytes ({:.2} MB)", peak_memory, peak_memory as f64 / (1024.0 * 1024.0));
