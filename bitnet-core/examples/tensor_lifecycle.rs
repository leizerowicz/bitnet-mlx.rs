...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    drop(f32_clone);
    println!(
        "   After dropping clone - ref count: {}",
        f32_tensor.ref_count()
    );
    ...
    println!(
        "   Converted to Candle tensor: shape={:?}, dtype={:?}",
        candle_tensor.shape(),
        candle_tensor.dtype()
    );
    ...
    println!(
        "   After access - idle time: {}s",
        handle.idle_time_seconds()?
    );
    ...
    println!(
        "   After dropping tensor - weak handle valid: {}",
        weak_handle.is_valid()
    );
    ...
    println!(
        "   Before cleanup - active allocations: {}",
    ...
    println!(
        "   After cleanup - active allocations: {}",
        final_metrics.active_allocations
    );
    println!(
        "   Total deallocated: {} bytes",
        final_metrics.total_deallocated
    );
