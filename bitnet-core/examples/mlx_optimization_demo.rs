...
fn run_mlx_demo() -> Result<(), Box<dyn std::error::Error>> {
    use bitnet_core::mlx::{
        BitNetMlxDevice, FusionPattern, GraphBuilder, MlxAutoTuner, MlxBatchOptimizer,
        MlxKernelFusion, MlxMemoryOptimizer, MlxProfiler, MlxTensorCache, Operation,
    };
    use mlx_rs::Array;
    ...
    if let Some(result) = fusion.try_fuse(&operations, &arrays) {
        match result {
            Ok(fused_result) => {
                println!(
                    "  Fusion successful! Result: {:?}",
                    fused_result.as_slice::<f32>()
                );
            }
            Err(e) => {
    ...
    if let Some(result) = fusion.try_fuse(&matmul_ops, &matmul_arrays) {
        match result {
            Ok(fused_result) => {
                println!(
                    "  Fusion successful! Result shape: {:?}",
                    fused_result.shape()
                );
            }
            Err(e) => {
    ...
    if let Some(cached_tensor) = cache.get("weights_layer1") {
        println!(
            "Retrieved cached tensor: {:?}",
            cached_tensor.as_slice::<f32>()
        );
    }
    ...
    let benchmark_fn = |batch_size: usize| -> Result<Duration, anyhow::Error> {
        ...
        Ok(Duration::from_nanos(
            (base_time.as_nanos() as f64 / efficiency) as u64,
        ))
    };

    println!("Finding optimal batch size...");
    let optimal_batch_size =
        batch_optimizer.find_optimal_batch_size("vector_ops", 64, benchmark_fn)?;
    ...
    let pooled_tensor =
        memory_optimizer.get_or_create_tensor(&[64, 64], mlx_rs::Dtype::Float32, &device)?;
