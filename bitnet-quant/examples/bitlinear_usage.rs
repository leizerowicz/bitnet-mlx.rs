...
use bitnet_quant::quantization::{
    QuantizationConfig, QuantizationPrecision, QuantizationStrategy, TernaryMethod,
};
...
fn main() -> Result<()> {
    ...
    let input =
        Tensor::randn(0.0f32, 1.0f32, &input_shape, &device)?.to_dtype(candle_core::DType::F32)?;
    ...
    let bias_input =
        Tensor::randn(0.0f32, 1.0f32, &[2, 512], &device)?.to_dtype(candle_core::DType::F32)?;
    ...
    let custom_input =
        Tensor::randn(0.0f32, 1.0f32, &[8, 256], &device)?.to_dtype(candle_core::DType::F32)?;
    let custom_output = custom_layer.forward(&custom_input)?;
    println!(
        "   ✓ Custom quantization output shape: {:?}",
        custom_output.shape()
    );
    ...
    let test_input =
        Tensor::randn(0.0f32, 1.0f32, &[1, 128], &device)?.to_dtype(candle_core::DType::F32)?;
    ...
    let new_weights =
        Tensor::randn(0.0f32, 0.1f32, &[256, 128], &device)?.to_dtype(candle_core::DType::F32)?;
    ...
    if let Ok(memory_stats) = update_layer.memory_stats() {
        ...
        println!(
            ...
            memory_stats.total_deallocated
        );
        println!(
    ...
    let batch_input =
        Tensor::randn(0.0f32, 1.0f32, &[16, 10, 32], &device)?.to_dtype(candle_core::DType::F32)?;
