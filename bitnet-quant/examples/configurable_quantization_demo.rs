...
fn create_sample_weights_1d(device: &Device) -> Result<Tensor, Box<dyn std::error::Error>> {
    let data: Vec<f32> = vec![
        2.5, -1.8, 0.3, -0.1, 0.0, 1.2, -2.1, 0.8, -0.4, 1.5, -0.9, 0.2, -1.3, 0.7, -0.2, 1.1,
        -0.6, 0.4, -1.7, 0.9,
    ];
...
fn demo_one_five_eight_bit_quantization(
    weights: &Tensor,
    device: &Device,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (method, method_name) in ternary_methods {
        ...
        for &factor in &threshold_factors {
            ...
            println!(
                "    📈 Distribution: +1: {positive_count}, 0: {zero_count}, -1: {negative_count}"
            );
        }
    }
...
fn demo_performance_comparison(
    ...
    for (precision, name) in precisions {
        let mut scheme =
            QuantizationSchemeFactory::create_from_precision(precision, device.clone());
...
fn demo_scheme_factory(
    ...
    println!(
        "✅ 1.58-bit scheme: {} bytes, {:.2}x compression",
    ...
    let mut custom_scheme =
        QuantizationSchemeFactory::create_custom_scheme(custom_config, device.clone());
    let custom_result = custom_scheme.quantize_tensor(weights)?;
    println!(
        "✅ Custom 4-bit scheme: {} bytes, {:.2}x compression",
    ...
    println!(
        "✅ 8-bit from precision: {} bytes, {:.2}x compression",
