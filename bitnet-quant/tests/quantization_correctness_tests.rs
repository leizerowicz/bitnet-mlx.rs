...
fn create_test_tensor(device: &Device, pattern: &str) -> Tensor {
    match pattern {
        ...
        "outliers" => Tensor::new(&[0.1f32, 0.2, 100.0, 0.3, -100.0, 0.4, 0.5, 0.6], device).unwrap(),
        "small_values" => Tensor::new(&[1e-6f32, -1e-6, 1e-7, -1e-7, 1e-8, -1e-8, 0.0, 1e-5], device).unwrap(),
        "large_values" => Tensor::new(&[1000.0f32, -1000.0, 500.0, -500.0, 2000.0, -2000.0, 1500.0, -1500.0], device).unwrap(),
...
fn test_activation_quantization_mathematical_correctness() {
    ...
    let test_patterns = vec!["uniform", "sparse", "outliers", "small_values", "large_values"];
    let precisions = vec![
    ...
    for pattern in test_patterns {
        for precision in &precisions {
            ...
            let abs_max = activations.abs().unwrap().max_all().unwrap().to_scalar::<f32>().unwrap();
...
fn test_quantization_error_bounds() {
    ...
    for (i, weights) in test_data.iter().enumerate() {
        ...
        let weight_quantizer = create_ternary_quantizer(TernaryMethod::MeanThreshold, Some(0.7)).unwrap();
        ...
        let scales_broadcast = quantized_activations.scales.broadcast_as(quantized_activations.values.shape()).unwrap();
        let dequantized_activations = quantized_activations.values.to_dtype(DType::F32).unwrap()
...
fn test_quantization_scale_factor_correctness() {
    ...
    for (data, expected_max) in test_cases {
        ...
        assert_abs_diff_eq!(quantized_activations.stats.scale_factor, expected_max, epsilon = 1e-6);
    }
}
...
fn test_quantization_sparsity_preservation() {
    ...
    let quantized_values = quantized.values.flatten_all().unwrap().to_vec1::<f32>().unwrap();
...
fn test_quantization_numerical_stability() {
    ...
    let edge_cases = vec![
        ...
        ("all_negative_ones", create_test_tensor(&device, "negative_ones")),
    ];
...
fn test_quantization_determinism() {
    ...
    for i in 1..activation_results.len() {
        assert_eq!(activation_results[0], activation_results[i], "Activation quantization is not deterministic");
    }
}
...
fn test_quantization_shape_preservation() {
    ...
    for shape_dims in shapes {
        ...
        let data: Vec<f32> = (0..total_elements).map(|i| (i as f32 - total_elements as f32 / 2.0) / 10.0).collect();
...
fn test_quantization_error_propagation() {
    ...
    for i in 1..errors.len() {
        assert!(errors[i] < errors[i-1] * 5.0, "Error increased too much: {} -> {}", errors[i-1], errors[i]);
    }
}
...
fn test_quantization_compression_ratios() {
    ...
    let test_tensor = Tensor::new(&[1.0f32, -1.0, 0.5, -0.5, 0.0, 2.0, -2.0, 1.5], &device).unwrap();
