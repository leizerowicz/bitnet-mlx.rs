...
fn test_dynamic_precision_adjustment() {
    ...
    let adjustment = controller.adjust_precision_dynamically(&high_error_stats).unwrap();
    ...
    let adjustment = controller.adjust_precision_dynamically(&low_compression_stats).unwrap();
    if let Some(adj) = adjustment {
    ...
    let adjustment = controller.adjust_precision_dynamically(&good_stats).unwrap();
...
fn test_enhanced_config_builder() {
    ...
    let bitnet_config = EnhancedQuantizationConfigBuilder::bitnet_optimized().build().unwrap();
    assert_eq!(bitnet_config.base.precision, QuantizationPrecision::OneFiveFiveBit);
    assert!(bitnet_config.auto_optimization);
    
    let perf_config = EnhancedQuantizationConfigBuilder::performance_optimized().build().unwrap();
    assert_eq!(perf_config.base.precision, QuantizationPrecision::OneBit);
    assert_eq!(perf_config.precision_control.dynamic_adjustment.strategy, AdjustmentStrategy::Aggressive);
    
    let acc_config = EnhancedQuantizationConfigBuilder::accuracy_optimized().build().unwrap();
    assert_eq!(acc_config.base.precision, QuantizationPrecision::FourBit);
    assert_eq!(acc_config.precision_control.dynamic_adjustment.strategy, AdjustmentStrategy::Conservative);
}

#[test]
fn test_config_conversion() {
    let config = EnhancedQuantizationConfigBuilder::bitnet_optimized().build().unwrap();
    ...
    assert_eq!(activation_config.base.precision, QuantizationPrecision::OneFiveFiveBit);
...
fn test_custom_metrics() {
    ...
    assert!(config.custom_metrics.contains(&"custom_metric_1".to_string()));
    assert!(config.custom_metrics.contains(&"custom_metric_2".to_string()));
}