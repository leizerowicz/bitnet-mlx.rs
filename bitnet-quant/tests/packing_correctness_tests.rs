...
fn create_test_weights(pattern: &str, size: usize) -> Vec<i8> {
    match pattern {
        ...
        "random_ternary" => {
            (0..size).map(|i| match i % 3 { 0 => -1, 1 => 0, 2 => 1, _ => 0 }).collect()
        }
        _ => vec![0i8; size],
    }
}
...
fn test_unpacked_weights_validation() {
    ...
    assert!(packer.validate_unpacked_weights(&wrong_length_weights, &packed).is_err());
    
    // Test invalid unpacked weights (invalid values)
    let invalid_weights = vec![-2i8, 0, 1, 3, 0, 1];
    assert!(packer.validate_unpacked_weights(&invalid_weights, &packed).is_err());
}
...
fn test_compression_ratio_accuracy() {
    ...
    for strategy in strategies {
        ...
        assert!(estimate_error < 0.5, 
            "Strategy {:?}: estimate {:.2} vs actual {:.2}, error {:.2}",
            strategy, estimate.compression_ratio, actual_ratio, estimate_error);
        
        // Verify other estimate fields
        assert_eq!(estimate.original_size_bytes, weights.len());
        assert!(estimate.packed_size_bytes > 0);
        assert_eq!(estimate.memory_saved_bytes, 
                  estimate.original_size_bytes.saturating_sub(estimate.packed_size_bytes));
        
        if estimate.original_size_bytes > 0 {
            let expected_percentage = (estimate.memory_saved_bytes as f32 / 
                                     estimate.original_size_bytes as f32) * 100.0;
            assert_abs_diff_eq!(estimate.savings_percentage, expected_percentage, epsilon = 0.1);
        }
    }
}
...
fn test_strategy_recommendation_logic() {
    let test_cases = vec![
        ("very_sparse", vec![0i8; 100], TernaryPackingStrategy::CompressedSparse),
        ("moderately_sparse", create_test_weights("sparse", 50), TernaryPackingStrategy::RunLengthEncoded),
        ("base3_optimal", vec![-1i8, 0, 1, -1, 0], TernaryPackingStrategy::Base3Packed),
        ("dense_general", create_test_weights("alternating", 20), TernaryPackingStrategy::BitPacked2Bit),
    ];
