...
use bitnet_quant::quantization::{activations::*, mixed_precision::*, packing::*, weights::*};
...
mod boundary_value_tests {
    ...
    fn test_extreme_tensor_values() {
        ...
        let extreme_values = vec![f32::MAX, f32::MIN, 0.0, -0.0, f32::EPSILON, -f32::EPSILON];

        let weights =
            Tensor::from_slice(&extreme_values, (extreme_values.len(),), &device).unwrap();
        let config = WeightQuantizationConfig::default();
    ...
    fn test_activation_boundary_values() {
        ...
        let activations =
            Tensor::from_slice(&boundary_values, (boundary_values.len(),), &device).unwrap();
...
mod resource_exhaustion_tests {
    ...
    fn test_packing_memory_efficiency() {
        ...
        for strategy in strategies {
            ...
            assert!(
                unpack_result.is_ok(),
                "Unpacking failed for strategy {strategy:?}"
            );
...
mod error_propagation_tests {
    ...
    fn test_corruption_detection_error_handling() {
        ...
        let corruption_types: Vec<_> = reports
            .iter()
...
mod stress_tests {
    ...
    fn test_quantization_performance_consistency() {
        ...
        for size in sizes {
            ...
            if times.len() > 1 {
                let ratio =
                    times.last().unwrap().as_secs_f64() / times[times.len() - 2].as_secs_f64();
    ...
    fn test_packing_performance_consistency() {
        ...
        for strategy in strategies {
            ...
            assert!(
                pack_time.as_millis() < 1000,
                "Packing too slow for {:?}: {}ms",
                strategy,
                pack_time.as_millis()
            );
            assert!(
                unpack_time.as_millis() < 1000,
                "Unpacking too slow for {:?}: {}ms",
                strategy,
                unpack_time.as_millis()
            );
        }
    }
...
mod integration_error_tests {
    ...
    fn test_end_to_end_error_handling() {
        ...
        let quantized_activations =
            absmax_quantize_activations(&activations, &device, None).unwrap();
        ...
        // 4. Test packing
        let ternary_weights: Vec<i8> = quantized_weights
            .values
            .to_vec1::<f32>()
            .unwrap()
            .iter()
            .map(|&x| x.clamp(-1.0, 1.0) as i8)
            .collect();
        ...
        let serious_corruptions: Vec<_> = reports
            .iter()
            .filter(|r| {
                matches!(
                    r.severity,
                    CorruptionSeverity::Severe | CorruptionSeverity::Critical
                )
            })
            .collect();
            ...
            Ok(quantized) => {
                ...
                for &val in &data {
                    assert!(
                        ...
                        "Quantized value {val} out of range"
                    );
                }
            }
            Err(e) => {
