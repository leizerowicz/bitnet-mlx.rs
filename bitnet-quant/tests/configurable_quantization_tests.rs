...
fn test_quantization_scheme_factory() {
    ...
    assert_eq!(
        from_precision_result.precision,
        QuantizationPrecision::EightBit
    );
}
...
fn test_quantization_error_bounds() {
    ...
    for precision in precisions {
        ...
        if matches!(
            ...
            assert!(
                error <= previous_error || error < 0.1,
...
fn test_custom_thresholds() {
    ...
    let mut scheme =
        ConfigurableQuantizationScheme::new(config_with_custom_thresholds, device.clone());
    let quantized = scheme.quantize_tensor(&input).unwrap();
