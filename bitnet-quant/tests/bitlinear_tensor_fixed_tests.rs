...
fn create_test_tensor(shape: &[usize], device: Device) -> TensorIntegrationResult<BitNetTensor> {
    ...
    BitNetTensor::random(shape, BitNetDType::F32, Some(device))
        .map_err(|e| bitnet_quant::tensor_integration::TensorIntegrationError::TensorOp { 
...
mod tests {
    ...
    fn test_quantization_precision() -> TensorIntegrationResult<()> {
        ...
        assert_eq!(precision_map.get(&QuantizationPrecision::OneBit), Some(&"1-bit".to_string()));
        Ok(())
    }
