...
impl BitLinear {
        ...
        if let Some(ref cache) = self.cache {
            ...
            if let Some(entry) = cache_guard.get(&self.layer_name) {
                ...
                let weights_guard = self.weights.read()
                    .map_err(|_| BitLinearError::MemoryError("Failed to acquire weights read lock".to_string()))?;
    ...
    fn quantize_and_cache_weights(&self) -> BitLinearResult<QuantizedWeight> {
        ...
        let quantizer_guard = self.quantizer.lock()
    ...
    pub fn cache_stats(&self) -> Option<crate::bitlinear::cache::CacheStats> {
        self.cache.as_ref().and_then(|cache| {
            cache.lock().ok().map(|guard| guard.stats())
        })
    }
