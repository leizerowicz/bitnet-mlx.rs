...
impl LazyQuantizer {
    ...
    fn try_get_cached(
        ...
        if let Some(entry) = cache.get_mut(layer_name) {
            ...
            if entry.state.is_quantized() && self.is_entry_valid_for_weights(entry, weights)? {
                ...
                if let (Some(ref quantized), Some(ref scales)) = (&entry.quantized_weights, &entry.scales) {
    ...
    fn set_quantization_state(&self, layer_name: &str, state: QuantizationState) -> BitLinearResult<()> {
        let mut cache = self.cache.write()
            .map_err(|_| BitLinearError::cache_lock_error("Failed to acquire cache lock"))?;
