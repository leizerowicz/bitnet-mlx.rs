...
impl CacheEntry {
    ...
    pub fn new(
        ...
        let memory_usage =
            estimate_memory_usage(&quantized_weights) + estimate_memory_usage(&scales);
...
impl WeightCacheManager {
    ...
    pub fn new(config: CacheConfig, memory_pool: Arc<HybridMemoryPool>) -> BitLinearResult<Self> {
        Ok(Self {
            config,
    ...
    fn ensure_cache_capacity(&mut self, key: &str, entry: &CacheEntry) -> BitLinearResult<()> {
        ...
        if needs_capacity || needs_memory {
            if !self.config.enable_lru_eviction {
                return Err(BitLinearError::cache_lock_error(
            ...
            let entries_to_evict = if needs_capacity {
                ...
                let avg_entry_size = if current_size > 0 {
                    current_memory / current_size
                } else {
                    entry_memory
                };
    ...
    fn maybe_auto_cleanup(&mut self) -> BitLinearResult<()> {
        let should_cleanup = {
            let last_cleanup = self.last_cleanup.lock().map(|t| *t).unwrap_or(0);
...
mod tests {
    ...
    fn test_cache_eviction() {
        ...
        for i in 0..3 {
            ...
            let entry =
                CacheEntry::new(quantized, scales, weights, format!("layer_{}", i)).unwrap();
