...
use crate::simd::{
    SimdTernaryOps, SimdMatrixOps, SimdPackingOps,
    vectorized_ternary_quantize, vectorized_ternary_dequantize,
    vectorized_matrix_multiply, vectorized_pack_ternary, vectorized_unpack_ternary,
    detect_simd_capabilities, SimdConfig, AlignedVec
};
...
fn test_simd_capability_detection() {
    ...
    if caps.has_simd() {
        println!("SIMD support available with {}-byte vectors", caps.vector_size);
    } else {
...
fn test_matrix_vector_multiplication() {
    ...
    // SIMD computation
    ops.ternary_matrix_vector(&a, &x, &mut y_simd, m, k, scale).unwrap();
...
fn test_rle_compression() {
    ...
    println!("RLE size: {}, Regular size: {}", rle_encoded.len(), regular_packed_size);
...
fn test_performance_consistency() {
    ...
    for size in &sizes {
        ...
        println!("Size {}: SIMD {:?}, Scalar {:?}, Speedup: {:.2}x", 
                size, duration_simd, duration_scalar, 
...
mod integration_tests {
    ...
    fn test_full_quantization_pipeline() {
        ...
        let original_weights: Vec<f32> = (0..1000)
            .map(|i| ((i as f32) - 500.0) / 100.0)
            .collect();
        ...
        vectorized_unpack_ternary(&packed_weights, &mut unpacked_weights, quantized_weights.len()).unwrap();
        
        let mut dequantized_weights = vec![0.0f32; original_weights.len()];
        vectorized_ternary_dequantize(&unpacked_weights, &mut dequantized_weights, threshold).unwrap();
