        ...
        for i in 0..m {
            for j in (0..n).step_by(8) {
                ...
                for kk in (0..k_vec).step_by(8) {
                    ...
                    let a_vals_i64 = std::ptr::read_unaligned(a_ternary.as_ptr().add(i * k + kk) as *const i64);
                    let a_vals_i8 = _mm_set1_epi64x(a_vals_i64);
                    let a_vals_i16 = _mm_unpacklo_epi8(a_vals_i8, _mm_cmplt_epi8(a_vals_i8, _mm_setzero_si128()));
                    let a_vals_i32_lo = _mm_unpacklo_epi16(a_vals_i16, _mm_cmplt_epi16(a_vals_i16, _mm_setzero_si128()));
                    let a_vals_i32_hi = _mm_unpackhi_epi16(a_vals_i16, _mm_cmplt_epi16(a_vals_i16, _mm_setzero_si128()));
        ...
        for i in 0..m {
            ...
            for kk in (0..k_vec).step_by(8) {
                ...
                if kk + 4 < k_vec {
                    ...
                    let x_vals2_hi = if kk + 8 < k { _mm_loadu_ps(x.as_ptr().add(kk + 8)) } else { _mm_setzero_ps() };
                    let x_vals2 = _mm256_insertf128_ps(_mm256_castps128_ps256(x_vals2_lo), x_vals2_hi, 1);
    ...
    unsafe fn ternary_gemv_sse2(
        ...
        for i in 0..m {
            ...
            for kk in (0..k_vec).step_by(4) {
                ...
                let a_vals_i32 = _mm_cvtsi32_si128(*(a_ternary.as_ptr().add(i * k + kk) as *const i32));
                let a_vals_i16 = _mm_unpacklo_epi8(a_vals_i32, _mm_cmplt_epi8(a_vals_i32, _mm_setzero_si128()));
                let a_vals_i32_full = _mm_unpacklo_epi16(a_vals_i16, _mm_cmplt_epi16(a_vals_i16, _mm_setzero_si128()));
...
mod tests {
    ...
    fn test_ternary_matrix_vector() {
        ...
        let ops = SimdMatrixOps::new();
        ops.ternary_matrix_vector(&a, &x, &mut y, m, k, 1.0).unwrap();
    ...
    fn test_ternary_matrix_multiply() {
        ...
        let ops = SimdMatrixOps::new();
        ops.ternary_matrix_multiply(&a, &b, &mut c, m, k, n, 1.0).unwrap();
    ...
    fn test_scalar_vs_simd_consistency() {
        ...
        // Compute with scalar
        ops.ternary_gemm_scalar(&a, &b, &mut c_scalar, m, k, n, 2.0).unwrap();
        
        // Compute with SIMD
        ops.ternary_matrix_multiply(&a, &b, &mut c_simd, m, k, n, 2.0).unwrap();
        ...
        for (i, (&scalar, &simd)) in c_scalar.iter().zip(c_simd.iter()).enumerate() {
            ...
            if (scalar - simd).abs() > 1e-5 {
                panic!("Mismatch at position {}: scalar={}, simd={}", i, scalar, simd);
            }
        }
    }
    ...
    fn test_large_matrix_performance() {
        ...
        ops.ternary_matrix_multiply(&a, &b, &mut c, m, k, n, 1.5).unwrap();
        let duration = start.elapsed();
        
        println!("Large matrix multiply ({} x {} x {}) took: {:?}", m, k, n, duration);
