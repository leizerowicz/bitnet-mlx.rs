...
pub trait TernaryPacker: Send + Sync {
    ...
    fn pack(&self, weights: &[i8], config: &TernaryPackingConfig) -> Result<PackedTernaryWeights, QuantizationError>;
    ...
    fn pack_with_validation(&self, weights: &[i8], config: &TernaryPackingConfig) -> Result<PackedTernaryWeights, QuantizationError> {
    ...
    fn unpack_with_validation(&self, packed: &PackedTernaryWeights) -> Result<Vec<i8>, QuantizationError> {
    ...
    fn validate_input_weights(&self, weights: &[i8]) -> Result<(), QuantizationError> {
        if weights.is_empty() {
            return Err(QuantizationError::InvalidInput("Input weights are empty".to_string()));
        }
    ...
    fn validate_unpacked_weights(&self, unpacked: &[i8], packed: &PackedTernaryWeights) -> Result<(), QuantizationError> {
        if unpacked.len() != packed.metadata.element_count {
            return Err(QuantizationError::InvalidInput(
                format!("Unpacked length {} doesn't match expected {}",
                       unpacked.len(), packed.metadata.element_count)
            ));
        }
    ...
    fn add_integrity_data(&self, packed: &mut PackedTernaryWeights) -> Result<(), QuantizationError> {
    ...
    fn verify_integrity_data(&self, packed: &PackedTernaryWeights) -> Result<(), QuantizationError> {
...
impl TernaryPacker for BitPacked2BitPacker {
    fn pack(&self, weights: &[i8], config: &TernaryPackingConfig) -> Result<PackedTernaryWeights, QuantizationError> {
        ...
        let padding = if weights.len() % 4 != 0 { 4 - (weights.len() % 4) } else { 0 };
...
impl TernaryPacker for Base3PackedPacker {
    fn pack(&self, weights: &[i8], config: &TernaryPackingConfig) -> Result<PackedTernaryWeights, QuantizationError> {
        ...
        let padding = if weights.len() % 5 != 0 { 5 - (weights.len() % 5) } else { 0 };
...
impl TernaryPacker for ByteAlignedPacker {
    ...
    fn unpack(&self, packed: &PackedTernaryWeights) -> Result<Vec<i8>, QuantizationError> {
        let weights: Vec<i8> = packed.data.iter()
...
impl TernaryPacker for RunLengthEncodedPacker {
    fn pack(&self, weights: &[i8], config: &TernaryPackingConfig) -> Result<PackedTernaryWeights, QuantizationError> {
        ...
        for &weight in &weights[1..] {
            if weight == current_value && count < 255 { // Limit count to fit in u8
                count += 1;
            } else {
...
impl TernaryPacker for HybridPacker {
    ...
    fn unpack(&self, packed: &PackedTernaryWeights) -> Result<Vec<i8>, QuantizationError> {
        ...
        while offset < packed.data.len() && weights.len() < packed.metadata.element_count {
            ...
            let block_data_size = u16::from_le_bytes([
                packed.data[offset + 1], packed.data[offset + 2]
            ]) as usize;
            offset += 3;
            
            if offset + block_data_size > packed.data.len() {
                return Err(QuantizationError::InvalidInput("Corrupted hybrid data".to_string()));
            }
...
impl HybridPacker {
    ...
    fn find_best_strategy_for_block(&self, weights: &[i8], config: &TernaryPackingConfig) -> Result<(TernaryPackingStrategy, PackedTernaryWeights), QuantizationError> {
        let strategies = [
            ...
            (TernaryPackingStrategy::RunLengthEncoded, Box::new(RunLengthEncodedPacker) as Box<dyn TernaryPacker>),
            (TernaryPackingStrategy::CompressedSparse, Box::new(CompressedSparsePacker) as Box<dyn TernaryPacker>),
        ];
    ...
    fn estimate_best_strategy_for_block(&self, weights: &[i8], config: &TernaryPackingConfig) -> (TernaryPackingStrategy, PackingSavingsEstimate) {
        let strategies = [
            (TernaryPackingStrategy::BitPacked2Bit, Box::new(BitPacked2BitPacker) as Box<dyn TernaryPacker>),
            (TernaryPackingStrategy::Base3Packed, Box::new(Base3PackedPacker) as Box<dyn TernaryPacker>),
            (TernaryPackingStrategy::RunLengthEncoded, Box::new(RunLengthEncodedPacker) as Box<dyn TernaryPacker>),
    ...
    fn unpack_block_with_strategy(&self, strategy_byte: u8, data: &[u8]) -> Result<Vec<i8>, QuantizationError> {
        let strategy = match strategy_byte {
            ...
            _ => return Err(QuantizationError::InvalidInput("Unknown packing strategy".to_string())),
        };
        ...
        let estimated_element_count = match strategy {
            ...
            TernaryPackingStrategy::CompressedSparse => {
                if data.len() >= 4 {
                    let total_elements = u32::from_le_bytes([data[0], data[1], data[2], data[3]]) as usize;
        ...
        match strategy {
            ...
            TernaryPackingStrategy::Hybrid => Err(QuantizationError::InvalidInput("Nested hybrid packing not supported".to_string())),
        }
    }
}
...
impl TernaryPackerFactory {
    ...
    pub fn auto_select_strategy(weights: &[i8], config: &TernaryPackingConfig) -> TernaryPackingStrategy {
    ...
    pub fn pack_optimal(weights: &[i8], config: &TernaryPackingConfig) -> Result<PackedTernaryWeights, QuantizationError> {
...
pub mod packing_utils {
    ...
    pub fn tensor_to_ternary(tensor: &Tensor) -> Result<Vec<i8>, QuantizationError> {
        ...
        let ternary: Vec<i8> = data.iter().map(|&f| {
            if f > 0.5 { 1 }
            else if f < -0.5 { -1 }
            else { 0 }
        }).collect();
    ...
    pub fn ternary_to_tensor(ternary: &[i8], shape: &Shape, device: &Device) -> Result<Tensor, QuantizationError> {
...
mod tests {
    ...
    fn test_strategy_recommendation() {
        ...
        assert_eq!(packing_utils::recommend_strategy(&dense_weights), TernaryPackingStrategy::BitPacked2Bit);
        
        // Sparse weights
        let sparse_weights = vec![0i8, 0, 0, 0, 0, 0, 0, 1, 0, 0];
        assert_eq!(packing_utils::recommend_strategy(&sparse_weights), TernaryPackingStrategy::CompressedSparse);
        
        // Base-3 optimal
        let base3_weights = vec![-1i8, 0, 1, -1, 0]; // Length 5
        assert_eq!(packing_utils::recommend_strategy(&base3_weights), TernaryPackingStrategy::Base3Packed);
    }
    ...
    fn test_compression_ratios() {
        ...
        for strategy in strategies {
            ...
            println!("Strategy {:?}: compression ratio = {:.2}", strategy, estimate.compression_ratio);
