...
pub enum RecoveryAction {
    ...
    UseFallback { fallback_strategy: TernaryPackingStrategy },
    /// Re-pack the data with a different strategy
    RepackData { suggested_strategy: TernaryPackingStrategy },
...
impl Default for CorruptionDetector {
    fn default() -> Self {
        ...
        detector.register_validator(TernaryPackingStrategy::BitPacked2Bit, Box::new(BitPacked2BitValidator));
        detector.register_validator(TernaryPackingStrategy::Base3Packed, Box::new(Base3PackedValidator));
        detector.register_validator(TernaryPackingStrategy::RunLengthEncoded, Box::new(RunLengthEncodedValidator));
        detector.register_validator(TernaryPackingStrategy::CompressedSparse, Box::new(CompressedSparseValidator));
...
impl CorruptionDetector {
    ...
    pub fn new(enable_checksums: bool, enable_deep_validation: bool, max_corruption_ratio: f32) -> Self {
        let mut detector = Self {
            enable_checksums,
    ...
    pub fn register_validator(&mut self, strategy: TernaryPackingStrategy, validator: Box<dyn StrategyValidator>) {
        self.strategy_validators.insert(strategy, validator);
    }
    
    /// Perform comprehensive corruption detection on packed data
    pub fn detect_corruption(&self, packed: &PackedTernaryWeights) -> Result<Vec<CorruptionReport>, QuantizationError> {
    ...
    fn validate_basic_structure(&self, packed: &PackedTernaryWeights) -> Result<Vec<CorruptionReport>, QuantizationError> {
        let mut reports = Vec::new();
        ...
        if packed.data.len() < expected_min_size {
            reports.push(CorruptionReport {
                ...
                recovery_suggestions: vec![RecoveryAction::ManualIntervention {
                    instructions: "Data appears truncated. Check source data integrity.".to_string(),
                }],
                context: HashMap::new(),
            });
        }
    ...
    fn validate_metadata(&self, packed: &PackedTernaryWeights) -> Result<Vec<CorruptionReport>, QuantizationError> {
        let mut reports = Vec::new();
        ...
        if packed.metadata.element_count == 0 && !packed.data.is_empty() {
            reports.push(CorruptionReport {
                ...
                recovery_suggestions: vec![RecoveryAction::AutoRepair {
                    description: "Attempt to infer element count from data size and strategy".to_string(),
                }],
                context: HashMap::new(),
            });
        }
        ...
        if let Some(expected_padding) = self.calculate_expected_padding(packed) {
            if packed.metadata.padding != expected_padding {
                reports.push(CorruptionReport {
                    ...
                    corrupted_length: None,
                    recovery_suggestions: vec![RecoveryAction::AutoRepair {
                        description: "Recalculate padding based on element count and strategy".to_string(),
                    }],
                    context: HashMap::new(),
                });
            }
    ...
    fn validate_checksum(&self, packed: &PackedTernaryWeights) -> Result<Option<CorruptionReport>, QuantizationError> {
            ...
            if stored_checksum_bytes.len() == 4 {
                ...
                if stored_checksum != calculated_checksum {
                    return Ok(Some(CorruptionReport {
                        ...
                        corrupted_length: None,
                        recovery_suggestions: vec![RecoveryAction::ManualIntervention {
                            instructions: "Checksum mismatch indicates data corruption. Verify source data.".to_string(),
                        }],
                        context: HashMap::new(),
                    }));
                }
    ...
    fn validate_strategy_specific(&self, packed: &PackedTernaryWeights) -> Result<Vec<CorruptionReport>, QuantizationError> {
    ...
    fn validate_deep(&self, packed: &PackedTernaryWeights) -> Result<Vec<CorruptionReport>, QuantizationError> {
        let mut reports = Vec::new();
    ...
    fn calculate_corruption_ratio(&self, reports: &[CorruptionReport], packed: &PackedTernaryWeights) -> f32 {
        if reports.is_empty() || packed.data.is_empty() {
            return 0.0;
        }
        
        let total_corrupted_bytes: usize = reports
            .iter()
            .filter_map(|r| r.corrupted_length)
            .sum();
    ...
    fn attempt_partial_unpack(&self, packed: &PackedTernaryWeights, max_elements: usize) -> Result<Vec<i8>, QuantizationError> {
...
impl StrategyValidator for Base3PackedValidator {
    fn validate(&self, packed: &PackedTernaryWeights) -> Result<Vec<CorruptionReport>, QuantizationError> {
        ...
        for (i, &byte) in packed.data.iter().enumerate() {
            if byte > 242 { // 3^5 - 1 = 242 is the maximum valid value
                reports.push(CorruptionReport {
                    corruption_type: CorruptionType::InvalidValues {
            ...
            if chunk.len() == 2 {
                ...
                if count == 0 {
                    reports.push(CorruptionReport {
                        ...
                        severity: CorruptionSeverity::Severe,
                        confidence: 1.0,
                        byte_offset: Some(chunk.as_ptr() as usize - packed.data.as_ptr() as usize + 1),
                        corrupted_length: Some(1),
                        recovery_suggestions: vec![RecoveryAction::AutoRepair {
    ...
    fn validate(&self, packed: &PackedTernaryWeights) -> Result<Vec<CorruptionReport>, QuantizationError> {
        ...
        for i in 0..nnz.min(packed.data.len().saturating_sub(values_offset)) {
            ...
            if value_offset < packed.data.len() {
                ...
                if value > 2 { // Ternary values should be 0, 1, or 2
                    reports.push(CorruptionReport {
                        corruption_type: CorruptionType::InvalidValues {
                            invalid_bytes: vec![value],
                            positions: vec![i],
    ...
    fn validate(&self, packed: &PackedTernaryWeights) -> Result<Vec<CorruptionReport>, QuantizationError> {
        ...
        while offset < packed.data.len() {
            ...
            let block_size = u16::from_le_bytes([
                packed.data[offset + 1], packed.data[offset + 2]
            ]) as usize;
            
            // Validate strategy byte
            if strategy_byte > 6 { // Assuming 7 strategies (0-6)
                reports.push(CorruptionReport {
                    corruption_type: CorruptionType::InvalidValues {
...
impl CorruptionDetector {
    ...
    pub fn add_checksum(&self, packed: &mut PackedTernaryWeights) {
        if self.enable_checksums {
            ...
            packed.metadata.extra_data.insert(
                "crc32".to_string(),
    ...
    pub fn attempt_repair(&self, packed: &mut PackedTernaryWeights, reports: &[CorruptionReport]) -> Result<usize, QuantizationError> {
        let mut repairs_made = 0;
        
        for report in reports {
            match &report.corruption_type {
                CorruptionType::InvalidValues { invalid_bytes, positions, .. } => {
                ...
                CorruptionType::PaddingCorruption { expected_padding, .. } => {
        ...
        for report in reports {
            match report.severity {
                CorruptionSeverity::Minor | CorruptionSeverity::Moderate => {
                    if report.recovery_suggestions.iter().any(|s| matches!(s, RecoveryAction::AutoRepair { .. })) {
                        plan.auto_repairable.push(report.clone());
                    }
                }
                CorruptionSeverity::Severe => {
                    if report.recovery_suggestions.iter().any(|s| matches!(s, RecoveryAction::UseFallback { .. })) {
...
impl RecoveryPlan {
    ...
    pub fn total_issues(&self) -> usize {
        self.auto_repairable.len() + self.requires_fallback.len() + self.requires_manual_intervention.len()
    }
}
...
mod tests {
    ...
    fn test_basic_structure_validation() {
        ...
        assert!(matches!(reports[0].corruption_type, CorruptionType::SizeMismatch { .. }));
    }
    ...
    fn test_checksum_validation() {
        ...
        assert!(matches!(report.unwrap().corruption_type, CorruptionType::ChecksumMismatch { .. }));
    }
    ...
    fn test_bit_packed_validator() {
        ...
        assert!(!reports.is_empty());
        assert!(matches!(reports[0].corruption_type, CorruptionType::InvalidValues { .. }));
    }
