...
impl CalibrationTensor {
    ...
    pub fn process_dataset(&mut self) -> TensorIntegrationResult<CalibrationResults> {
        ...
        let dataset_len = {
            let dataset = self.current_dataset.as_ref()
            ...
            if dataset.samples.len() < self.config.num_samples {
                return Err(TensorIntegrationError::from(CalibrationError::InsufficientSamples {
    ...
    fn process_batch(&mut self, batch: &[BitNetTensor]) -> TensorIntegrationResult<CalibrationResults> {
    ...
    fn analyze_sample(&mut self, sample: &BitNetTensor) -> TensorIntegrationResult<SampleStatistics> {
        let candle_tensor = sample.to_candle_tensor()
    ...
    fn compute_moments(&self, tensor: &CandleTensor) -> TensorIntegrationResult<StatisticalMoments> {
    ...
    fn analyze_distribution(&self, tensor: &CandleTensor) -> TensorIntegrationResult<DistributionAnalysis> {
    ...
    /// Finalize statistical analysis
    fn finalize_statistics(&mut self, results: &mut CalibrationResults) -> TensorIntegrationResult<()> {
    ...
    /// Compute recommended quantization parameters
    fn compute_quantization_recommendations(&self, results: &CalibrationResults) -> TensorIntegrationResult<QuantizationRecommendations> {
        let moments = results.aggregate_moments.as_ref()
...
mod tests {
    ...
    fn test_calibration_config_default() {
        ...
        assert_eq!(config.target_precision, QuantizationPrecision::OneFiveFiveBit);
