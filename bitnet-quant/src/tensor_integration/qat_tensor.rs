...
impl StraightThroughEstimator {
    ...
    pub fn binary_quantization_ste(
        ...
        let candle_tensor = input.to_candle_tensor()
    ...
    pub fn weight_quantization_ste(
        ...
        let params = self.layer_parameters.entry(layer_id.to_string())
            .or_default();
        
        let candle_tensor = weights.to_candle_tensor()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to get candle tensor: {e}") 
            })?;
    ...
    pub fn multi_bit_quantization_ste(
        ...
        let shifted = candle_tensor.add(&CandleTensor::new(1.0f32, input.device())
            .map_err(|e| TensorIntegrationError::TensorOp { 
        ...
        let scaled = shifted.mul(&CandleTensor::new(scale, input.device())
            .map_err(|e| TensorIntegrationError::TensorOp { 
        ...
        let dequantized = quantized_scaled
            .div(&CandleTensor::new(scale, input.device())
                .map_err(|e| TensorIntegrationError::TensorOp { 
            ...
            .sub(&CandleTensor::new(1.0f32, input.device())
                .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn apply_fake_binary_quantization(&self, tensor: &CandleTensor) -> TensorIntegrationResult<CandleTensor> {
    ...
    fn apply_fake_ternary_quantization(&self, tensor: &CandleTensor, threshold: f32) -> TensorIntegrationResult<CandleTensor> {
        let abs_input = tensor.abs()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to compute absolute value: {e}") 
            })?;
        
        let threshold_tensor = CandleTensor::new(threshold, tensor.device())
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn apply_true_ternary_quantization(&self, tensor: &CandleTensor, threshold: f32) -> TensorIntegrationResult<CandleTensor> {
        // Same as fake quantization but with additional processing
        self.apply_fake_ternary_quantization(tensor, threshold)
    }
    
    fn add_quantization_noise(&self, tensor: &CandleTensor) -> TensorIntegrationResult<CandleTensor> {
        ...
        let noise = CandleTensor::randn(0.0, self.config.noise_level as f64, noise_shape, tensor.device())
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn apply_temperature_scaling(&self, tensor: &CandleTensor, temperature: f32) -> TensorIntegrationResult<CandleTensor> {
        if temperature == 1.0 {
            return Ok(tensor.clone());
        }
        
        let temp_tensor = CandleTensor::new(temperature, tensor.device())
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to create temperature tensor: {e}") 
            })?;
        
        tensor.div(&temp_tensor)
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn update_quantization_stats(
        ...
        self.quantization_stats.layer_quality.insert(layer_id.to_string(), 1.0 - mse);
...
impl QATTensorOps {
    ...
    pub fn apply_qat(
        ...
        match tensor_type {
            QATTensorType::Weight => {
                self.ste.weight_quantization_ste(input, layer_id)
            },
            QATTensorType::Activation => {
                self.ste.activation_quantization_ste(input, layer_id)
            },
            QATTensorType::Binary => {
                self.ste.binary_quantization_ste(input, layer_id)
            },
            QATTensorType::Ternary(threshold) => {
                self.ste.ternary_quantization_ste(input, threshold, layer_id)
            },
        }
    }
