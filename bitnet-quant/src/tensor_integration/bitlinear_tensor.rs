...
use bitnet_core::{
    BitNetTensor, BitNetDType, TensorShape
};
...
pub enum BitLinearTensorError {
    ...
    WeightShapeMismatch { expected: Vec<usize>, actual: Vec<usize> },
...
impl BitLinearTensorOpsImpl {
    ...
    pub fn quantize_weights(&self, weights: BitNetTensor) -> TensorIntegrationResult<WeightQuantizationTensor> {
        ...
        let quantized_weights = QuantizedTensor::from_bitnet_tensor(weights.clone(), quantized_config)?;
    ...
    pub fn quantize_activations(&self, activations: BitNetTensor) -> TensorIntegrationResult<ActivationQuantizationTensor> {
        let mut activation_stats = ActivationStats::default();
        
        // Compute activation statistics
        let candle_tensor = activations.to_candle_tensor()
        ...
        let quantized_activations = if self.config.activation_quantization.precision != QuantizationPrecision::OneFiveFiveBit {
            // For non-ternary activations, quantize immediately
            Some(QuantizedTensor::from_bitnet_tensor(activations.clone(), quantized_config)?)
        } else {
    ...
    pub fn forward(
        ...
        let matmul_result = self.quantized_linear(
            &activation_tensor,
            weight_tensor
        )?;
    ...
    pub fn apply_layernorm(
        ...
        let input_tensor = input.to_candle_tensor()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to get input tensor: {e}") 
            })?;
        
        let weight_tensor = layernorm.weight.to_candle_tensor()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to get LayerNorm weight: {e}") 
            })?;
        
        let bias_tensor = layernorm.bias.to_candle_tensor()
            .map_err(|e| TensorIntegrationError::TensorOp { 
        ...
        let centered = input_tensor.broadcast_sub(&mean)
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to center input: {e}") 
            })?;
        
        let eps_tensor = variance.ones_like()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to create eps tensor: {e}") 
            })?
            .mul(&candle_core::Tensor::new(layernorm.eps, input.device())
                .map_err(|e| TensorIntegrationError::TensorOp { 
        ...
        let std = variance.add(&eps_tensor)
            .map_err(|e| TensorIntegrationError::TensorOp { 
        ...
        let scaled = normalized.broadcast_mul(&weight_tensor)
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to apply weight: {e}") 
            })?;
        
        let result = scaled.broadcast_add(&bias_tensor)
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    pub fn quantized_linear(
        ...
        let result_quantized = quantized_activations.quantized_matmul(&weight_tensor.quantized_weights)?;
    ...
    pub fn apply_residual_connection(
        ...
        if !main_path.shape().is_broadcast_compatible(residual.shape()) {
            return Err(TensorIntegrationError::ShapeMismatch {
                message: format!("Residual connection shape mismatch: {:?} vs {:?}",
                               main_path.shape(), residual.shape()),
            });
        }
        
        main_path.add(residual)
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    pub fn create_layernorm_integration(
        ...
        let pool = bitnet_core::tensor::memory_integration::get_global_memory_pool()
            .ok_or_else(|| TensorIntegrationError::Memory(
                bitnet_core::MemoryError::InsufficientMemory { 
                    size: 0,
                }
            ))?;
    ...
    pub fn mixed_precision_forward(
        ...
        self.forward(&processed_input, weight_tensor, bias, layernorm, residual_input)
    }
    ...
    fn optimize_for_cpu_simd(&self, tensor: &BitNetTensor) -> TensorIntegrationResult<BitNetTensor> {
...
impl MixedPrecisionBitLinearOps {
    ...
    pub fn forward(
        ...
        // Perform quantized linear transformation
        let result = quantized_input.matmul(&quantized_weights.to_bitnet_tensor()?)
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn apply_weight_precision(&self, weights: &QuantizedTensor) -> TensorIntegrationResult<QuantizedTensor> {
        // Apply weight-specific precision transformations
        Ok(weights.clone())
    }
    
    fn apply_activation_precision(&self, input: &BitNetTensor) -> TensorIntegrationResult<BitNetTensor> {
        // Apply activation-specific precision transformations
        Ok(input.clone())
    }
    
    fn update_precision_stats(&mut self, duration: f32, result: &BitNetTensor) -> TensorIntegrationResult<()> {
        // Update weight precision stats
        *self.precision_stats.operations_by_precision
            .entry(self.weight_precision)
            .or_insert(0) += 1;
            
        *self.precision_stats.avg_time_by_precision
            .entry(self.weight_precision)
    ...
    pub fn get_precision_summary(&self) -> PrecisionSummary {
        PrecisionSummary {
            total_operations: self.precision_stats.operations_by_precision
                .values()
                .sum(),
            avg_duration: self.precision_stats.avg_time_by_precision
                .values()
                .sum::<f32>() / self.precision_stats.avg_time_by_precision.len() as f32,
    ...
    fn calculate_memory_efficiency(&self) -> f32 {
        ...
        let efficiency: f32 = self.precision_stats.operations_by_precision
            .iter()
            .map(|(precision, count)| {
