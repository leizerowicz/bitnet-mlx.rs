...
use bitnet_core::{
    BitNetTensor, 
    TensorShape, auto_select_device
};
...
impl PerformanceOptimizedPrecision {
    ...
    pub fn get_policy() -> PrecisionPolicy {
        ...
        custom_precision.insert(LayerType::FeedForward, QuantizationPrecision::OneFiveFiveBit);
...
pub trait PrecisionTensorOps {
    ...
    fn apply_mixed_precision(&self, tensor: &BitNetTensor, layer_type: LayerType) -> TensorIntegrationResult<QuantizedTensor>;
    
    /// Convert between precisions
    fn convert_precision(&self, tensor: &QuantizedTensor, target_precision: QuantizationPrecision) -> TensorIntegrationResult<QuantizedTensor>;
    
    /// Select optimal precision for layer
    fn select_precision(&self, layer_type: LayerType, tensor_shape: &TensorShape) -> TensorIntegrationResult<QuantizationPrecision>;
    
    /// Update precision policy based on performance
    fn update_precision_policy(&mut self, performance_data: &PerformanceData) -> TensorIntegrationResult<()>;
}
...
impl MixedPrecisionTensorOpsImpl {
    ...
    fn initialize_precision_assignments(config: &PrecisionConfig) -> HashMap<LayerType, QuantizationPrecision> {
    ...
    pub fn apply_precision_policy(&mut self, policy: PrecisionPolicy) -> TensorIntegrationResult<()> {
        match policy {
    ...
    fn get_accuracy_optimized_assignments() -> HashMap<LayerType, QuantizationPrecision> {
        ...
        assignments.insert(LayerType::FeedForward, QuantizationPrecision::OneFiveFiveBit);
        assignments.insert(LayerType::LayerNorm, QuantizationPrecision::EightBit);
    ...
    fn setup_dynamic_precision(&mut self, _config: DynamicPrecisionConfig) -> TensorIntegrationResult<()> {
    ...
    pub fn get_precision_for_layer(&self, layer_type: LayerType) -> QuantizationPrecision {
        self.precision_assignments.get(&layer_type)
            .cloned()
            .unwrap_or(self.config.default_precision)
    }
    
    /// Update performance metrics
    pub fn update_performance_metrics(&mut self, data: &PerformanceData) -> TensorIntegrationResult<()> {
    ...
    fn adjust_precision_based_on_performance(&mut self, data: &PerformanceData) -> TensorIntegrationResult<()> {
        ...
        for (layer_id, metrics) in &data.layer_metrics {
            if let Ok(layer_type) = Self::parse_layer_type(layer_id) {
                ...
                let new_precision = if metrics.execution_time > 10.0 && metrics.accuracy_contribution < 0.1 {
...
impl PrecisionTensorOps for MixedPrecisionTensorOpsImpl {
    ...
    fn select_precision(&self, layer_type: LayerType, _tensor_shape: &TensorShape) -> TensorIntegrationResult<QuantizationPrecision> {
...
impl PerformanceMonitor {
    ...
    pub fn update_metrics(&mut self, data: &PerformanceData) -> TensorIntegrationResult<()> {
        ...
        self.add_metric(PerformanceMetric::Latency, data.model_metrics.inference_time);
        
        // Update memory metrics
        self.add_metric(PerformanceMetric::Memory, data.model_metrics.memory_usage as f32);
...
mod tests {
    ...
    fn test_layer_type_parsing() {
        assert_eq!(MixedPrecisionTensorOpsImpl::parse_layer_type("bitlinear_0"), Ok(LayerType::BitLinear));
        assert_eq!(MixedPrecisionTensorOpsImpl::parse_layer_type("attention_layer"), Ok(LayerType::Attention));
        assert_eq!(MixedPrecisionTensorOpsImpl::parse_layer_type("ffn_block"), Ok(LayerType::FeedForward));
        assert_eq!(MixedPrecisionTensorOpsImpl::parse_layer_type("layernorm_1"), Ok(LayerType::LayerNorm));
    }
    ...
    fn test_precision_assignments() {
        ...
        assert_eq!(ops.get_precision_for_layer(LayerType::BitLinear), QuantizationPrecision::OneFiveFiveBit);
        assert_eq!(ops.get_precision_for_layer(LayerType::LayerNorm), QuantizationPrecision::EightBit);
        assert_eq!(ops.get_precision_for_layer(LayerType::Embedding), QuantizationPrecision::FourBit);
    }
    ...
    fn test_performance_monitor() {
        ...
        assert_eq!(monitor.get_average_metric(PerformanceMetric::Latency), Some(5.0));
        assert_eq!(monitor.get_average_metric(PerformanceMetric::Accuracy), Some(0.95));
    }
}
