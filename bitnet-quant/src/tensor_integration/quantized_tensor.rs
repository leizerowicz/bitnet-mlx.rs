...
use bitnet_core::{
    BitNetTensor, BitNetDType, MemoryResult, MemoryError, 
    TensorShape, HybridMemoryPool
};
...
use super::bitnet_ops::{
    TernaryTensorRepresentation, TernaryArithmetic, QuantizedArithmetic
};
...
impl QuantizedTensor {
    ...
    pub fn from_bitnet_tensor(
        ...
        let device = config.device.clone().unwrap_or_else(|| tensor.device().clone());
        ...
        let storage = match config.precision {
            ...
            _ => {
                Self::create_general_storage(tensor, parameters, &config)?
            }
        };
    ...
    pub fn from_ternary_representation(
        ...
        let device = config.device.clone().unwrap_or_else(|| ternary.device.clone());
        let tensor_id = rand::random::<u64>();
    ...
    pub fn to_bitnet_tensor(&self) -> TensorIntegrationResult<BitNetTensor> {
        match &self.storage {
            QuantizedStorage::Dense { data, parameters } => {
                self.dequantize_dense(data, parameters)
            },
            QuantizedStorage::Sparse { indices, values, shape, parameters } => {
                self.dequantize_sparse(indices, values, shape, parameters)
            },
            QuantizedStorage::Packed { packed_data, shape, parameters, unpacking_info } => {
                self.dequantize_packed(packed_data, shape, parameters, unpacking_info)
            },
            QuantizedStorage::Ternary { ternary_repr, .. } => {
                self.dequantize_ternary(ternary_repr)
            },
        }
    }
    ...
    pub fn compression_ratio(&self) -> CompressionRatio {
        let original_size = match &self.storage {
            ...
            QuantizedStorage::Sparse { shape, parameters, .. } => {
                shape.total_elements() * Self::dtype_size(parameters.original_dtype)
            },
            QuantizedStorage::Packed { shape, parameters, .. } => {
                shape.total_elements() * Self::dtype_size(parameters.original_dtype)
            },
            QuantizedStorage::Ternary { ternary_repr, .. } => {
                ternary_repr.original_shape.total_elements() * Self::dtype_size(ternary_repr.original_dtype)
            },
        };
    ...
    fn compute_quantization_parameters(
        ...
        let candle_tensor = tensor.to_candle_tensor()
        ...
        match config.precision {
            QuantizationPrecision::OneFiveFiveBit => {
                let alpha = candle_tensor.abs().map_err(|e| TensorIntegrationError::TensorOp { 
                    message: format!("Failed to compute abs: {e}") 
                })?
                .mean_all().map_err(|e| TensorIntegrationError::TensorOp { 
                    message: format!("Failed to compute mean: {e}") 
                })?
                .to_scalar::<f32>().map_err(|e| TensorIntegrationError::TensorOp { 
            ...
            _ => {
                ...
                let (scale, zero_point) = Self::compute_scale_zero_point(min_val, max_val, config.precision);
    ...
    fn compute_tensor_range(tensor: &CandleTensor) -> TensorIntegrationResult<(f32, f32)> {
        let min_val = tensor.min(0).map_err(|e| TensorIntegrationError::TensorOp { 
            message: format!("Failed to compute min: {e}") 
        })?
        .min_keepdim(0).map_err(|e| TensorIntegrationError::TensorOp { 
        ...
        let max_val = tensor.max(0).map_err(|e| TensorIntegrationError::TensorOp { 
            message: format!("Failed to compute max: {e}") 
        })?
        .max_keepdim(0).map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn compute_scale_zero_point(min_val: f32, max_val: f32, precision: QuantizationPrecision) -> (f32, f32) {
    ...
    fn create_ternary_storage(
        ...
        let sign_tensor = candle_tensor.sign()
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn create_general_storage(
        ...
        let scale_tensor = candle_tensor.ones_like()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to create scale tensor: {e}") 
            })?
            .mul(&candle_core::Tensor::new(parameters.scale_zero_point.scale, tensor.device())
                .map_err(|e| TensorIntegrationError::TensorOp { 
        ...
        let quantized = candle_tensor.div(&scale_tensor)
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn compute_layout(storage: &QuantizedStorage) -> TensorIntegrationResult<QuantizedLayout> {
        match storage {
            QuantizedStorage::Dense { data, parameters } => {
                let memory_footprint = data.shape().total_elements() * Self::dtype_size(parameters.original_dtype);
                Ok(QuantizedLayout {
                    storage_type: QuantizedStorageType::Dense,
                    alignment: 64,
                    strides: data.shape().compute_strides().iter().map(|&s| s as usize).collect(),
                    is_contiguous: true,
                    memory_footprint,
                })
            },
            QuantizedStorage::Ternary { ternary_repr, .. } => {
                ...
                Ok(QuantizedLayout {
                    ...
                    alignment: 64,
                    strides: ternary_repr.original_shape.compute_strides().iter().map(|&s| s as usize).collect(),
                    is_contiguous: true,
                    memory_footprint,
                })
            },
    ...
    fn dequantize_dense(&self, data: &BitNetTensor, parameters: &QuantizationParameters) -> TensorIntegrationResult<BitNetTensor> {
        let candle_tensor = data.to_candle_tensor()
            .map_err(|e| TensorIntegrationError::TensorOp { 
        ...
        let scale_tensor = candle_tensor.ones_like()
            ...
            .mul(&candle_core::Tensor::new(parameters.scale_zero_point.scale, &self.device)
                .map_err(|e| TensorIntegrationError::TensorOp { 
                    message: format!("Failed to create scale scalar: {e}") 
                })?)
            .map_err(|e| TensorIntegrationError::TensorOp { 
        ...
        let dequantized = candle_tensor.mul(&scale_tensor)
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn dequantize_ternary(&self, ternary_repr: &TernaryTensorRepresentation) -> TensorIntegrationResult<BitNetTensor> {
        let values = ternary_repr.values.to_candle_tensor()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to get ternary values: {e}") 
            })?;
        
        let scales = ternary_repr.scales.to_candle_tensor()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to get scale values: {e}") 
            })?;
        
        let dequantized = values.broadcast_mul(&scales)
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn dequantize_sparse(&self, _indices: &BitNetTensor, _values: &BitNetTensor, _shape: &TensorShape, _parameters: &QuantizationParameters) -> TensorIntegrationResult<BitNetTensor> {
    ...
    fn dequantize_packed(&self, _packed_data: &Vec<u8>, _shape: &TensorShape, _parameters: &QuantizationParameters, _unpacking_info: &PackingInfo) -> TensorIntegrationResult<BitNetTensor> {
...
impl QuantizedArithmetic for QuantizedTensor {
    ...
    fn quantized_add(&self, other: &Self) -> TensorIntegrationResult<Self> {
        if !self.is_shapes_compatible(other) {
            return Err(TensorIntegrationError::ShapeMismatch {
                message: format!("Cannot add tensors with shapes {:?} and {:?}", 
                               self.shape(), other.shape()),
            });
        }
        
        match (&self.storage, &other.storage) {
            (QuantizedStorage::Dense { data: lhs, parameters: lhs_params }, 
             QuantizedStorage::Dense { data: rhs, parameters: rhs_params }) => {
            ...
            _ => {
                ...
                let result_full = lhs_full.add(&rhs_full)
                    .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn quantized_mul(&self, other: &Self) -> TensorIntegrationResult<Self> {
        ...
        let lhs_full = self.to_bitnet_tensor()?;
        let rhs_full = other.to_bitnet_tensor()?;
        let result_full = lhs_full.mul(&rhs_full)
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn quantized_matmul(&self, other: &Self) -> TensorIntegrationResult<Self> {
        ...
        let lhs_full = self.to_bitnet_tensor()?;
        let rhs_full = other.to_bitnet_tensor()?;
        let result_full = lhs_full.matmul(&rhs_full)
            .map_err(|e| TensorIntegrationError::TensorOp { 
    ...
    fn quantized_scale(&self, scalar: f32) -> TensorIntegrationResult<Self> {
        ...
        let scaled = full_tensor.mul_scalar(scalar)
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to scale tensor: {e}") 
            })?;
...
impl TernaryArithmetic for QuantizedTensor {
    ...
    fn ternary_add(&self, other: &Self) -> TensorIntegrationResult<Self> {
        if let (Some(lhs_ternary), Some(rhs_ternary)) = 
            (self.get_ternary_representation(), other.get_ternary_representation()) {
            
            // Ternary addition using lookup table for {-1, 0, 1} + {-1, 0, 1}
            let lhs_values = lhs_ternary.values.to_candle_tensor()
                .map_err(|e| TensorIntegrationError::TensorOp { 
                    message: format!("Failed to get LHS ternary values: {e}") 
                })?;
            
            let rhs_values = rhs_ternary.values.to_candle_tensor()
                .map_err(|e| TensorIntegrationError::TensorOp { 
                    message: format!("Failed to get RHS ternary values: {e}") 
                })?;
            
            // Add ternary values (result may be outside {-1, 0, 1})
            let result_values = lhs_values.add(&rhs_values)
                .map_err(|e| TensorIntegrationError::TensorOp { 
                    message: format!("Failed to add ternary values: {e}") 
                })?;
            
            // Clamp to ternary range and rescale
            let clamped_values = result_values.clamp(-1.0, 1.0)
                .map_err(|e| TensorIntegrationError::TensorOp { 
                    message: format!("Failed to clamp ternary result: {e}") 
                })?;
            
            let result_tensor = BitNetTensor::from_candle_tensor(clamped_values, self.device.clone())
                .map_err(TensorIntegrationError::Memory)?;
            
            // Combine scales
            let combined_scale = (lhs_ternary.quantization_params.scale + rhs_ternary.quantization_params.scale) / 2.0;
            ...
            let result_ternary = TernaryTensorRepresentation {
                ...
                quantization_params: super::bitnet_ops::TernaryQuantizationParams {
                    ...
                    threshold: (lhs_ternary.quantization_params.threshold + rhs_ternary.quantization_params.threshold) / 2.0,
    ...
    fn ternary_mul(&self, other: &Self) -> TensorIntegrationResult<Self> {
        if let (Some(lhs_ternary), Some(rhs_ternary)) = 
            ...
            let result_values = lhs_values.mul(&rhs_values)
                .map_err(|e| TensorIntegrationError::TensorOp { 
                    message: format!("Failed to multiply ternary values: {e}") 
                })?;
            
            let result_tensor = BitNetTensor::from_candle_tensor(result_values, self.device.clone())
                .map_err(TensorIntegrationError::Memory)?;
            
            // Multiply scales
            let combined_scale = lhs_ternary.quantization_params.scale * rhs_ternary.quantization_params.scale;
            let scales = BitNetTensor::from_scalar(combined_scale, self.device.clone())
                .map_err(TensorIntegrationError::Memory)?;
    ...
    fn ternary_matmul(&self, other: &Self) -> TensorIntegrationResult<Self> {
        ...
        if let (Some(lhs_ternary), Some(rhs_ternary)) = 
            ...
            let result_values = lhs_ternary.values.matmul(&rhs_ternary.values)
                .map_err(|e| TensorIntegrationError::TensorOp { 
                    message: format!("Failed to perform matmul: {e}") 
                })?;
                
            // Combine scales for matrix multiplication
            let combined_scale = lhs_ternary.scales.mul(&rhs_ternary.scales)
                .map_err(|e| TensorIntegrationError::TensorOp { 
            ...
            self.dequantize_compute_requantize(|lhs| {
                let rhs_full = other.to_bitnet_tensor().map_err(|e| MemoryError::InternalError {
                    reason: e.to_string()
                })?;
                lhs.matmul(&rhs_full).map_err(|e| MemoryError::InternalError {
                    reason: e.to_string()
                })
            })
        }
    ...
    {
        let full_tensor = self.to_bitnet_tensor()?;
        let result = operation(&full_tensor)
            .map_err(TensorIntegrationError::Memory)?;
...
impl QuantizedTensor {
    fn add_dense_tensors(&self, lhs: &BitNetTensor, rhs: &BitNetTensor, lhs_params: &QuantizationParameters, rhs_params: &QuantizationParameters) -> TensorIntegrationResult<Self> {
        // Simplified dense addition - in practice would need proper scale handling
        let lhs_tensor = lhs.to_candle_tensor()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to get LHS tensor: {e}") 
            })?;
        
        let rhs_tensor = rhs.to_candle_tensor()
            .map_err(|e| TensorIntegrationError::TensorOp { 
                message: format!("Failed to get RHS tensor: {e}") 
            })?;
        
        let result_tensor = lhs_tensor.add(&rhs_tensor)
            .map_err(|e| TensorIntegrationError::TensorOp { 
