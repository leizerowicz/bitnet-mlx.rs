...
use crate::metrics::{
    cosine_similarity::CosineSimilarityCalculator, error_analysis::ErrorAnalyzer,
    mse::MSECalculator, safe_divide, sqnr::SQNRCalculator, tensor_to_vec, ErrorThresholds,
...
impl LayerWiseAnalyzer {
    ...
    pub fn analyze_layers(
    ...
    ) -> Result<LayerWiseAnalysisResult> {
        ...
        for (layer_name, data) in layer_data.iter() {
            ...
            let mitigations = self
                .error_analyzer
                .suggest_mitigation(&metrics, &self.thresholds);
        ...
        let optimization_plan =
            self.generate_optimization_plan(&layer_metrics, &sensitivity_scores);
    ...
    fn calculate_layer_metrics(
        ...
        let error_stats = self
            .error_analyzer
            .calculate_error_statistics(original, quantized)?;
    ...
    fn create_layer_rankings(
        &self,
        layer_metrics: &HashMap<String, QuantizationMetrics>,
    ) -> Vec<LayerRanking> {
        let mut rankings = Vec::new();

        // MSE ranking (lower is better)
        let mut mse_ranking: Vec<_> = layer_metrics
            .iter()
        ...
        let mut sqnr_ranking: Vec<_> = layer_metrics
            .iter()
        ...
        let mut cosine_ranking: Vec<_> = layer_metrics
            .iter()
    ...
    fn calculate_error_correlations(
        &self,
        layer_data: &HashMap<String, LayerData>,
        ...
        for layer1 in &layer_names {
            ...
            for layer2 in &layer_names {
                if layer1 != layer2 {
                    let correlation =
                        self.calculate_layer_correlation(&layer_data[layer1], &layer_data[layer2])?;
    ...
    fn calculate_layer_correlation(&self, data1: &LayerData, data2: &LayerData) -> Result<f32> {
        ...
        let numerator: f32 = error1_slice
            .iter()
            .zip(error2_slice.iter())
            .map(|(&e1, &e2)| (e1 - mean1) * (e2 - mean2))
            .sum();

        let variance1: f32 = error1_slice.iter().map(|&e| (e - mean1).powi(2)).sum();

        let variance2: f32 = error2_slice.iter().map(|&e| (e - mean2).powi(2)).sum();
    ...
    fn calculate_global_statistics(
        ...
        let best_mse_layer = values
            .iter()
            .min_by(|a, b| a.mse.partial_cmp(&b.mse).unwrap())
            .map(|m| m.layer_name.clone());
        let worst_mse_layer = values
            .iter()
            .max_by(|a, b| a.mse.partial_cmp(&b.mse).unwrap())
            .map(|m| m.layer_name.clone());

        let best_sqnr_layer = values
            .iter()
            .filter(|m| m.sqnr.is_finite())
    ...
    fn generate_optimization_plan(
        ...
        for (layer_name, sensitivity) in sensitivity_ranking {
            ...
            if sensitivity > &10.0 {
                ...
                optimization_strategies.insert(
                    layer_name.clone(),
                    vec![OptimizationStrategy::MaintainCurrentSettings],
                );
            }
        }
    ...
    fn assess_implementation_complexity(
        ...
        let high_sensitivity_count = sensitivity_ranking
            .iter()
    ...
    fn identify_problematic_layers(
        &self,
        layer_metrics: &HashMap<String, QuantizationMetrics>,
        thresholds: &ErrorThresholds,
    ) -> Vec<ProblematicLayer> {
    ...
    fn assess_issue_severity(
        ...
        let critical_issues = issues
            .iter()
            .filter(|&&issue| matches!(issue, QualityIssue::HighMSE | QualityIssue::LowSQNR))
            .count();
        let high_issues = issues
            ...
            .filter(|&&issue| {
                matches!(
                    issue,
                    QualityIssue::PoorSimilarity | QualityIssue::HighRelativeError
                )
            })
            .count();
    ...
    fn get_recommended_actions(&self, issues: &[QualityIssue]) -> Vec<String> {
        ...
        for issue in issues {
            match issue {
                QualityIssue::HighMSE => {
                    actions.push("Increase bit width or improve calibration".to_string())
                }
                QualityIssue::LowSQNR => {
                    actions.push("Use asymmetric quantization or add regularization".to_string())
                }
                QualityIssue::PoorSimilarity => {
                    actions.push("Enable mixed precision or adjust quantization method".to_string())
                }
                QualityIssue::HighRelativeError => actions
                    .push("Apply channel-wise quantization or improve scale factors".to_string()),
                QualityIssue::ExcessiveBitFlips => {
                    actions.push("Increase bit width or use smoother quantization".to_string())
                }
            }
        }
    ...
    pub fn analyze_temporal_trends(
        &self,
        temporal_data: HashMap<u64, HashMap<String, LayerData>>,
        ...
        for layer_name in layer_names {
            ...
            for &timestamp in &sorted_timestamps {
                if let Some(layer_data) = temporal_data
                    .get(&timestamp)
                    .and_then(|data| data.get(&layer_name))
                {
    ...
    fn assess_overall_quality_change(
        &self,
        first: &QuantizationMetrics,
        last: &QuantizationMetrics,
    ) -> f32 {
    ...
    fn calculate_volatility(&self, evolution: &[(u64, QuantizationMetrics)]) -> f32 {
        ...
        let mse_variance = mse_values
            .iter()
            .map(|&x| (x - mean_mse).powi(2))
            .sum::<f32>()
            / mse_values.len() as f32;
