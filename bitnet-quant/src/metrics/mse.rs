...
pub fn calculate_mse_streaming(original: &Tensor, quantized: &Tensor, chunk_size: usize) -> Result<f32> {
    if original.shape() != quantized.shape() {
        return Err(CandleError::Msg(format!(
...
impl MSECalculator {
    ...
    pub fn calculate_comprehensive(&self, original: &Tensor, quantized: &Tensor) -> Result<MSEMetrics> {
    ...
    fn calculate_nmse_internal(&self, original: &Tensor, _quantized: &Tensor, mse: f32) -> Result<f32> {
        let original_power = original.powf(2.0)?.mean_all()?.to_scalar::<f32>()?;
        // Safe divide: return 0 if denominator is too small, else divide
        Ok(if original_power < f32::EPSILON { 0.0 } else { mse / original_power })
    }

    /// Calculate layer-wise MSE for multi-layer analysis
    pub fn calculate_layerwise(&self, layer_outputs: &[(String, Tensor, Tensor)]) -> Result<Vec<(String, MSEMetrics)>> {
    ...
    pub fn calculate_evolution(&self, original: &Tensor, quantized_series: &[Tensor]) -> Result<Vec<f32>> {
    ...
    pub fn calculate_weighted(&self, original: &Tensor, quantized: &Tensor, weights: &Tensor) -> Result<f32> {
        ...
        Ok(if total_weights < f32::EPSILON { 0.0 } else { total_weighted_error / total_weights })
    }
}
