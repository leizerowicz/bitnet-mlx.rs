...
impl VisualizationEngine {
    ...
    pub fn create_error_dashboard(&self, analysis: &LayerWiseAnalysisResult) -> Result<VisualizationDashboard> {
        ...
        let sensitivity_chart = self.create_sensitivity_ranking_chart(&analysis.sensitivity_ranking)?;
        dashboard.add_visualization(sensitivity_chart);

        // 3. Error correlation heatmap
        if !analysis.error_correlations.is_empty() {
            let correlation_heatmap = self.create_correlation_heatmap(&analysis.error_correlations)?;
        ...
        if !analysis.problematic_layers.is_empty() {
            let problems_viz = self.create_problematic_layers_chart(&analysis.problematic_layers)?;
    ...
    fn create_layer_comparison_chart(&self, analysis: &LayerWiseAnalysisResult) -> Result<Visualization> {
        ...
        for (layer_name, metrics) in &analysis.layer_metrics {
            data_points.push(DataPoint {
                ...
                values: vec![
                    ...
                    ("Cosine Similarity".to_string(), metrics.cosine_similarity.into()),
                    ("Relative Error".to_string(), metrics.relative_error.into()),
                ],
                metadata: Some(format!("Layer: {}, Timestamp: {}", layer_name, metrics.timestamp)),
            });
        }
    ...
    fn create_sensitivity_ranking_chart(&self, sensitivity_ranking: &[(String, f32)]) -> Result<Visualization> {
        let mut data_points = Vec::new();
        
        for (layer_name, sensitivity) in sensitivity_ranking.iter().take(20) { // Top 20 most sensitive
            data_points.push(DataPoint {
                label: layer_name.clone(),
    ...
    fn create_correlation_heatmap(&self, correlations: &HashMap<String, HashMap<String, f32>>) -> Result<Visualization> {
        let mut data_points = Vec::new();
    ...
    fn create_statistics_overview(&self, stats: &crate::metrics::layer_wise::GlobalStatistics) -> Result<Visualization> {
        let data_points = vec![
            DataPoint {
            ...
            DataPoint {
                ...
                metadata: Some(format!("Global mean cosine similarity: {:.4}", stats.mean_cosine_similarity)),
            },
            DataPoint {
                label: "Number of Layers".to_string(),
                values: vec![("Value".to_string(), DataValue::Integer(stats.num_layers as i64))],
    ...
    fn create_problematic_layers_chart(&self, problematic_layers: &[crate::metrics::layer_wise::ProblematicLayer]) -> Result<Visualization> {
        let mut data_points = Vec::new();
        ...
        for layer in problematic_layers {
            ...
            data_points.push(DataPoint {
                ...
                values: vec![
                    ...
                    ("Issue Count".to_string(), DataValue::Integer(layer.issues.len() as i64)),
                ],
    ...
    fn create_optimization_plan_chart(&self, plan: &crate::metrics::layer_wise::OptimizationPlan) -> Result<Visualization> {
        let data_points = vec![
            DataPoint {
                label: "High Priority".to_string(),
                values: vec![("Count".to_string(), DataValue::Integer(plan.high_priority_layers.len() as i64))],
                metadata: Some(format!("High priority layers: {}", plan.high_priority_layers.len())),
            },
            DataPoint {
                label: "Medium Priority".to_string(),
                values: vec![("Count".to_string(), DataValue::Integer(plan.medium_priority_layers.len() as i64))],
                metadata: Some(format!("Medium priority layers: {}", plan.medium_priority_layers.len())),
            },
            DataPoint {
                label: "Low Priority".to_string(),
                values: vec![("Count".to_string(), DataValue::Integer(plan.low_priority_layers.len() as i64))],
                metadata: Some(format!("Low priority layers: {}", plan.low_priority_layers.len())),
            },
        ];
    ...
    pub fn create_error_histogram(&self, histogram: &ErrorHistogram, layer_name: &str) -> Result<Visualization> {
        let mut data_points = Vec::new();
        ...
        for (i, &count) in histogram.bins.iter().enumerate() {
            if i < histogram.bin_edges.len() - 1 {
                ...
                data_points.push(DataPoint {
                    ...
                    metadata: Some(format!("Range: [{:.6}, {:.6}], Count: {}", 
                        histogram.bin_edges[i], histogram.bin_edges[i + 1], count)),
                });
            }
        }
    ...
    pub fn create_temporal_visualization(&self, temporal_analysis: &TemporalAnalysis) -> Result<Visualization> {
        let mut data_points = Vec::new();
        ...
        for (layer_name, trend) in &temporal_analysis.layer_trends {
            data_points.push(DataPoint {
                label: layer_name.clone(),
                values: vec![
                    ("Quality Change".to_string(), trend.overall_quality_change.into()),
    ...
    pub fn create_sqnr_evolution_chart(&self, evolution: &SQNREvolution, layer_name: &str) -> Result<Visualization> {
        let mut data_points = Vec::new();
    ...
    pub fn create_similarity_evolution_chart(&self, evolution: &SimilarityEvolution, layer_name: &str) -> Result<Visualization> {
        let mut data_points = Vec::new();
    ...
    pub fn create_tensor_visualization(&self, original: &Tensor, quantized: &Tensor, layer_name: &str) -> Result<Visualization> {
                ...
                for j in 0..dims[1] {
                    ...
                    if idx < error_vec.len() {
                        data_points.push(DataPoint {
                            ...
                            metadata: Some(format!("Position: ({}, {}), Error: {:.6}", i, j, error_vec[idx])),
                        });
                    }
                }
            }
    ...
    fn generate_export_file(&self, visualization: &Visualization, format: &OutputFormat, filename: &str) -> Result<()> {
        match format {
            OutputFormat::JSON => {
    ...
    fn generate_svg(&self, _visualization: &Visualization) -> Result<String> {
        ...
        Ok(r#"<svg xmlns="http://www.w3.org/2000/svg" width="800" height="600">
...
</html>"#, visualization.title, visualization.title))
    }
...
mod tests {
    ...
    use crate::metrics::layer_wise::{LayerWiseAnalysisResult, GlobalStatistics};
    use std::collections::HashMap;

    fn create_test_analysis() -> LayerWiseAnalysisResult {
        let mut layer_metrics = HashMap::new();
        layer_metrics.insert("layer1".to_string(), crate::metrics::QuantizationMetrics {
            mse: 0.001,
            sqnr: 40.0,
        ...
        layer_metrics.insert("layer2".to_string(), crate::metrics::QuantizationMetrics {
            mse: 0.01,
            sqnr: 20.0,
        ...
        LayerWiseAnalysisResult {
            ...
            optimization_plan: crate::metrics::layer_wise::OptimizationPlan {
                ...
                implementation_complexity: crate::metrics::layer_wise::ImplementationComplexity::Low,
            },
    ...
    fn test_layer_comparison_chart() -> Result<()> {
        ...
        assert!(matches!(chart.visualization_type, VisualizationType::GroupedBarChart));
    ...
    fn test_sensitivity_ranking_chart() -> Result<()> {
        ...
        assert_eq!(chart.data_points.len(), 2);
        assert!(matches!(chart.visualization_type, VisualizationType::HorizontalBarChart));
