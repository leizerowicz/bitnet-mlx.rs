...
fn benchmark_quantization(c: &mut Criterion) {
    ...
    let shapes = vec![vec![1024, 1024], vec![2048, 2048], vec![4096, 4096]];
...
fn benchmark_bitlinear(c: &mut Criterion) {
    ...
    let shapes = vec![vec![128, 512], vec![256, 1024], vec![512, 2048]];
...
fn benchmark_matmul(c: &mut Criterion) {
    ...
    let shapes = vec![vec![256, 512], vec![512, 1024], vec![1024, 2048]];
