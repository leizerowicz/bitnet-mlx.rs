...
use bitnet_core::tensor::ops::arithmetic::{add, add_scalar, mul};
use bitnet_core::tensor::ops::{simd_add_f32, simd_add_scalar_f32, simd_mul_f32, simd_sum_f32};
...
fn bench_element_wise_addition(c: &mut Criterion) {
    ...
    for &size in &config.tensor_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("simd_add", size), &size, |b, &size| {
            b.iter_batched(
                || {
                    let a = create_random_tensor(&[size], BitNetDType::F32, Some(device.clone()))
                        .expect("Failed to create tensor");
...
fn bench_element_wise_multiplication(c: &mut Criterion) {
    ...
    for &size in &config.tensor_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("scalar_mul", size), &size, |b, &size| {
            b.iter_batched(
                || {
                    let a = create_random_tensor(&[size], BitNetDType::F32, Some(device.clone()))
                        .expect("Failed to create tensor");
        ...
        // SIMD multiplication
        group.bench_with_input(BenchmarkId::new("simd_mul", size), &size, |b, &size| {
            b.iter_batched(
                || {
                    let a = create_random_tensor(&[size], BitNetDType::F32, Some(device.clone()))
                        .expect("Failed to create tensor");
    ...
    for &size in &config.tensor_sizes {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                b.iter(|| {
                    let tensor =
                        BitNetTensor::zeros(&[size], BitNetDType::F32, Some(device.clone()));
                    black_box(tensor)
                });
            },
        );

        group.bench_with_input(BenchmarkId::new("tensor_clone", size), &size, |b, &size| {
...
fn bench_broadcasting_operations(c: &mut Criterion) {
    ...
    for (lhs_shape, rhs_shape, name) in broadcasting_cases {
        let total_elements = lhs_shape
            .iter()
            .product::<usize>()
            .max(rhs_shape.iter().product::<usize>());
        group.throughput(Throughput::Elements(total_elements as u64));
        ...
        group.bench_with_input(
            ...
            |b, (lhs_shape, rhs_shape)| {
                b.iter_batched(
                    || {
                        let a =
                            create_random_tensor(lhs_shape, BitNetDType::F32, Some(device.clone()))
                                .expect("Failed to create tensor");
                        let b =
                            create_random_tensor(rhs_shape, BitNetDType::F32, Some(device.clone()))
                                .expect("Failed to create tensor");
                        (a, b)
                    },
                    |(a, b)| {
