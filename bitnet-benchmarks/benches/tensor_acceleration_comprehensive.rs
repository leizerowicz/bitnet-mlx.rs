...
#[cfg(feature = "mlx")]
use bitnet_core::mlx::{BitNetMlxDevice, MlxTensor, operations::BitNetMlxOps};
...
fn bench_mlx_matrix_multiplication(c: &mut Criterion) {
    ...
    println!("🚀 Running MLX acceleration benchmarks on: {:?}", mlx_device.device_type());
            ...
            group.bench_with_input(
                ...
                |bencher, &(rows, cols, dtype)| {
                    ...
                    for _ in 0..config.warmup_iterations {
                        let _ = BitNetMlxOps::matmul(&tensor_a, &tensor_b)
                            .expect("MLX matmul failed");
                    }
                    
                    bencher.iter(|| {
                        let result = BitNetMlxOps::matmul(&tensor_a, &tensor_b)
                            .expect("MLX matmul failed");
                        black_box(result)
                    });
                },
    ...
    for &size in &config.vector_sizes {
        ...
        group.bench_with_input(
            ...
            |bencher, &_size| {
                ...
                bencher.iter(|| {
                    let result = simd_mul_f32(&tensor_a, &tensor_b)
                        .expect("SIMD mul failed");
                    black_box(result)
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |bencher, &_size| {
                ...
                bencher.iter(|| {
                    let result = simd_sum_f32(&tensor_a)
                        .expect("SIMD sum failed");
                    black_box(result)
                });
            },
        );
    ...
    for &(rows, cols) in &test_sizes {
        ...
        if bitnet_core::mlx::is_mlx_available() {
            group.bench_with_input(
                ...
                |bencher, &(rows, cols)| {
                    let mlx_device = BitNetMlxDevice::default();
                    let tensor_a = MlxTensor::randn(&[rows, cols], BitNetDType::F32, mlx_device.clone())
                        .expect("Failed to create MLX tensor A");
                    let tensor_b = MlxTensor::randn(&[cols, rows], BitNetDType::F32, mlx_device.clone())
                        .expect("Failed to create MLX tensor B");
    ...
    for &size in &transfer_sizes {
        ...
        group.bench_with_input(
            ...
            |bencher, &size| {
                ...
                bencher.iter(|| {
                    let mlx_tensor = MlxTensor::from_slice(&cpu_data, &[size], BitNetDType::F32, mlx_device.clone())
        ...
        group.bench_with_input(
            ...
            |bencher, &size| {
                ...
                bencher.iter(|| {
                    let cpu_data = mlx_tensor.to_vec::<f32>().expect("Failed to convert MLX tensor to CPU data");
                    black_box(cpu_data)
                });
            },
        );
...
fn bench_memory_pool_acceleration(c: &mut Criterion) {
    ...
    println!("💾 Running memory pool acceleration benchmarks with {} iterations", config.iterations);
...
fn bench_configurable_iteration_performance(c: &mut Criterion) {
    ...
    println!("🔄 Running configurable iteration benchmarks with {} warmup and {} iterations", 
