...
fn benchmark_bit_packed_detailed(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        group.bench_with_input(BenchmarkId::new("simd", size), &packed, |b, packed| {
            b.iter(|| {
                let result = simd_unpacker.unpack(black_box(packed)).unwrap();
                black_box(result);
            });
        ...
        group.bench_with_input(BenchmarkId::new("scalar", size), &packed, |b, packed| {
            b.iter(|| {
                let result = scalar_unpacker.unpack(black_box(packed)).unwrap();
                black_box(result);
            });
...
fn benchmark_sparse_data(c: &mut Criterion) {
    ...
    for &size in &sizes {
        for &sparsity in &sparsity_levels {
            ...
            let packer =
                TernaryPackerFactory::create_packer(TernaryPackingStrategy::CompressedSparse);
            let packed = packer.pack(&weights, &config).unwrap();
...
fn benchmark_convenience_function(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        let original_packer =
            TernaryPackerFactory::create_packer(TernaryPackingStrategy::BitPacked2Bit);
        group.bench_with_input(
