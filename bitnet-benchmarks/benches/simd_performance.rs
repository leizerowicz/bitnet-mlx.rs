...
use bitnet_quant::simd::{
    ...
    detect_simd_capabilities, SimdTernaryOps
};
...
fn benchmark_ternary_packing(c: &mut Criterion) {
    ...
    for size in sizes.iter() {
        ...
        // Packing benchmark
        group.bench_with_input(
            BenchmarkId::new("pack_simd", size),
            size,
            |b, _| {
                b.iter(|| {
                    vectorized_pack_ternary(
                        black_box(&input),
        ...
        // Unpacking benchmark
        group.bench_with_input(
            BenchmarkId::new("unpack_simd", size),
            size,
            |b, _| {
                b.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("pack_scalar", size),
            size,
            |b, _| {
                b.iter(|| {
...
fn benchmark_absmean_computation(c: &mut Criterion) {
    ...
    for size in sizes.iter() {
        let values: Vec<f32> = (0..*size).map(|i| ((i as f32) - (*size as f32 / 2.0)) / 1000.0).collect();
