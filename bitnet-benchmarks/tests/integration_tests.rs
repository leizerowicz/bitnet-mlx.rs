...
use bitnet_benchmarks::{
    BenchmarkRunner, CandleOps, CandlePerformanceUtils, ComparisonConfig, PerformanceComparator,
};
...
fn test_candle_layer_norm() {
    ...
    let norm_result =
        CandleOps::layer_norm(&tensor, &[4], Some(&weight), Some(&bias), 1e-5).unwrap();
...
mod mlx_tests {
    ...
    fn test_mlx_bitlinear_forward() {
        ...
        let input =
            MlxTensor::randn(&[batch_size, input_size], BitNetDType::F32, device.clone()).unwrap();
        let weight =
            MlxTensor::randn(&[input_size, output_size], BitNetDType::F32, device.clone()).unwrap();
