...
use crate::comparison::{PerformanceMeasurement, ComparisonResult};
use anyhow::Result;
...
impl PerformanceVisualizer {
    ...
    pub fn generate_performance_chart(&self, measurements: &[PerformanceMeasurement]) -> Result<String> {
        ...
        for (i, (operation, group_measurements)) in operation_groups.iter().enumerate() {
            let avg_throughput: f64 = group_measurements.iter()
                .map(|m| m.throughput)
                .sum::<f64>() / group_measurements.len() as f64;
            ...
            svg.push_str(&format!(
                ...
                bar_x, bar_y, bar_width, bar_height, colors[i % colors.len()]
            ));
    ...
    pub fn generate_speedup_chart(&self, comparisons: &[ComparisonResult]) -> Result<String> {
        ...
        if !comparisons.is_empty() {
            ...
            for (i, comparison) in comparisons.iter().enumerate() {
                ...
                svg.push_str(&format!(
                    r#"<rect x="{}" y="{}" width="{}" height="{}" fill="{}" opacity="0.8"/>"#,
                    bar_x + 5, bar_y, bar_width - 10, bar_height, color
                ));
...
"#.to_string()
    ...
    fn generate_executive_summary(&self, measurements: &[PerformanceMeasurement], comparisons: &[ComparisonResult]) -> String {
        ...
        let best_speedup = comparisons.iter()
            .map(|c| c.speedup)
            .fold(0.0f64, |a, b| a.max(b));
        
        let success_rate = if !measurements.is_empty() {
            measurements.iter().filter(|m| m.success).count() as f64 / measurements.len() as f64 * 100.0
        } else {
            0.0
        };
...
"#, total_operations, avg_throughput, best_speedup, success_rate)
    ...
    fn generate_performance_charts(&self, measurements: &[PerformanceMeasurement]) -> String {
        let chart = self.generate_performance_chart(measurements).unwrap_or_default();
        format!(r#"
