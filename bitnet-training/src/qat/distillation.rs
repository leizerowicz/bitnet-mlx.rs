...
impl KnowledgeDistillation {
    ...
    pub fn compute_complete_loss(
        ...
        let feature_loss = if let (Some(student_feat), Some(teacher_feat)) = (student_features, teacher_features) {
    ...
    pub fn compute_kl_divergence_loss(&self, student_logits: &Tensor, teacher_logits: &Tensor) -> Result<Tensor> {
        // Convert to F32 and apply temperature scaling
        let temp_scale = 1.0 / self.config.temperature;
        let student_soft = student_logits.to_dtype(DType::F32)?.affine(temp_scale as f64, 0.0)?;
        let teacher_soft = teacher_logits.to_dtype(DType::F32)?.affine(temp_scale as f64, 0.0)?;
    ...
    pub fn compute_student_loss(&self, student_logits: &Tensor, targets: &Tensor) -> Result<Tensor> {
    ...
    pub fn compute_feature_matching_loss(
        ...
        for layer_name in &self.config.feature_layers {
            if let (Some(student_feat), Some(teacher_feat)) = 
                (student_features.get(layer_name), teacher_features.get(layer_name)) {
...
mod tests {
    ...
    fn test_complete_distillation_loss() -> Result<()> {
        ...
        let (loss, metrics) = kd.compute_complete_loss(
            &student_logits, 
            &teacher_logits, 
            &targets, 
            None, 
    ...
    fn test_teacher_student_pair() {
        let mut pair = TeacherStudentPair::new(
