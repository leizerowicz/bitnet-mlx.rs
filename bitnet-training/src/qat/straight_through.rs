...
impl StraightThroughEstimator {
    ...
    pub fn new(config: STEConfig, device: Device) -> Result<Self> {
        ...
        if config.variant == STEVariant::Learnable {
    ...
    pub fn forward(&mut self, input: &Tensor) -> Result<Tensor> {
        ...
        match self.config.variant {
            ...
            STEVariant::Learnable => self.learnable_forward(input),
        }
    }
    ...
    fn clipped_forward(&mut self, input: &Tensor) -> Result<Tensor> {
        ...
        let clipped_elements = diff_mask.to_dtype(input.dtype())?.sum_all()?.to_scalar::<f32>()? as usize;
    ...
    fn soft_forward(&mut self, input: &Tensor) -> Result<Tensor> {
        ...
        let temp_tensor = Tensor::new(self.config.temperature, input.device())?.to_dtype(input.dtype())?;
        let scaled_input = input.broadcast_mul(&temp_tensor)?;
        let soft_quantized = scaled_input.tanh()?;
        let range_tensor = Tensor::new(self.config.range, soft_quantized.device())?.to_dtype(input.dtype())?;
    ...
    fn binary_quantize(&self, input: &Tensor) -> Result<Tensor> {
        ...
        let sign = input.sign()?;
        let range_tensor = Tensor::new(self.config.range, input.device())?.to_dtype(input.dtype())?;
    ...
    fn ternary_quantize(&self, input: &Tensor) -> Result<Tensor> {
        ...
        let pos_val = Tensor::full(self.config.range, input.shape(), input.device())?.to_dtype(input.dtype())?;
        ...
        let neg_val = Tensor::full(-self.config.range, input.shape(), input.device())?.to_dtype(input.dtype())?;
    ...
    fn multi_bit_quantize(&self, input: &Tensor) -> Result<Tensor> {
        ...
        let quantized = quantized_indices.broadcast_mul(&step_size_tensor)?.broadcast_sub(&range_tensor)?;
    ...
    pub fn update_learnable_params(&mut self, scale_grad: &Tensor, zero_point_grad: &Tensor) -> Result<()> {
        if let (Some(scale), Some(zero_point)) = (&self.learnable_scale, &self.learnable_zero_point) {
            // Simple gradient descent update
            let lr_tensor = Tensor::new(self.config.learnable_lr, scale_grad.device())?.to_dtype(scale_grad.dtype())?;
...
pub fn quantize_with_ste(
    input: &Tensor,
    config: &STEConfig,
    device: &Device,
) -> Result<Tensor> {
    let mut ste = StraightThroughEstimator::new(config.clone(), device.clone())?;
    ste.forward(input)
}
...
impl MultiLayerSTE {
    ...
    pub fn get_or_create_layer_ste(&mut self, layer_name: &str) -> Result<&mut StraightThroughEstimator> {
        if !self.estimators.contains_key(layer_name) {
            let ste = StraightThroughEstimator::new(self.global_config.clone(), self.device.clone())?;
