...
pub trait QATOptimizer {
    ...
    fn step(&mut self, parameters: &mut HashMap<String, Tensor>, gradients: &HashMap<String, Tensor>) -> Result<()>;
...
impl QuantizationAwareAdam {
    ...
    fn apply_quantization_scaling(&self, param_name: &str, gradient: &Tensor) -> Result<Tensor> {
        ...
        let adaptive_scale = self.adaptive_scaling.get(param_name).cloned().unwrap_or(1.0);
        ...
        if let Some(stats) = self.quantization_stats.get(param_name) {
            ...
            match gradient.dtype() {
                ...
                dtype => return Err(candle_core::Error::UnexpectedDType { 
    ...
    fn update_adaptive_scaling(&mut self, param_name: &str, gradient_norm: f32) {
        let current_scale = self.adaptive_scaling.get(param_name).cloned().unwrap_or(1.0);
        ...
        self.adaptive_scaling.insert(param_name.to_string(), new_scale);
    }
}
    ...
    fn step(&mut self, parameters: &mut HashMap<String, Tensor>, gradients: &HashMap<String, Tensor>) -> Result<()> {
        ...
        for (param_name, parameter) in parameters.iter_mut() {
            if let Some(gradient) = clipped_gradients.get(param_name) {
                ...
                let grad_norm = scaled_gradient.sqr()?.sum_all()?.sqrt()?.to_scalar::<f32>()?;
                ...
                *velocity = (&velocity_term1 + &velocity_term2)?;
                ...
                let update = (&update_numerator / &velocity_sqrt)?.affine(self.learning_rate as f64, 0.0)?;
                
                // Apply weight decay if specified
                let final_update = if self.weight_decay > 0.0 {
                    let decay_term = parameter.affine((self.weight_decay * self.learning_rate) as f64, 0.0)?;
                    (&update + &decay_term)?
                } else {
                    update
                };
...
mod tests {
    ...
    fn create_test_parameters() -> Result<HashMap<String, Tensor>> {
        ...
        params.insert("weight1".to_string(), Tensor::from_slice(&[1.0, 2.0], (2,), &Device::Cpu)?);
        params.insert("weight2".to_string(), Tensor::from_slice(&[3.0, 4.0], (2,), &Device::Cpu)?);
        Ok(params)
    }

    fn create_test_gradients() -> Result<HashMap<String, Tensor>> {
        let mut grads = HashMap::new();
        grads.insert("weight1".to_string(), Tensor::from_slice(&[0.1, 0.2], (2,), &Device::Cpu)?);
        grads.insert("weight2".to_string(), Tensor::from_slice(&[0.3, 0.4], (2,), &Device::Cpu)?);
        Ok(grads)
    }
    ...
    fn test_quantization_stats_update() -> Result<()> {
        ...
        stats.insert("weight1".to_string(), STEStatistics {
