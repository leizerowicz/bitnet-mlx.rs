...
fn test_regularization(device: &Device) -> Result<()> {
    ...
    assert!(
        reg_value < 1000.0,
        "Regularization loss should be reasonable"
    );
...
fn test_state_tracking(device: &Device) -> Result<()> {
    ...
    println!(
        "  Regularization stats: Total = {:.6}",
        reg_stats.total_regularization
    );
...
fn test_optimizers(device: &Device) -> Result<()> {
    ...
    let param_diff = (updated_param - &initial_param)?
        .abs()?
        .sum_all()?
        .to_scalar::<f32>()?;
...
mod tests {
    ...
    fn test_qat_utilities_integration() {
        ...
        assert!(
            result.is_ok(),
            "QAT utilities test should pass: {:?}",
            result.err()
        );
    }
}
